...
// Pack stdlib
let t = glob.sync(path.resolve(__dirname, '..', 'src', 'stdlib', '**', '*.@(tact|fc)'));
let output: string = '';
output = 'let files: { [key: string]: string } = {};\n';
for (let f of t) {
    let code = fs.readFileSync(f).toString('base64');
    let name = f.replace(path.resolve(__dirname, '..', 'src', 'stdlib'), ''); // Thanks ChatGPT
    output += `files['${name}'] =\n`;
    let first = true;
    while (code.length > 0) {
        if (first) {
...
fs.writeFileSync(path.resolve(__dirname, '..', 'src', 'stdlib', 'stdlib.ts'), output);