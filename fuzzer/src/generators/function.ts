...
export class FunctionDef extends GenerativeEntity<AstFunctionDef> {
  ...
  public generateImpl(
    ...
    return fc.record<AstFunctionDef>({
      ...
      attributes: fc.constant(getAttributes(extraAttrs, this.kind, false)),
      name: fc.constant(this.name!),
      return: fc.constant(isUnit(returnTy) ? undefined : tyToAstType(returnTy)),
      params: fc.constant(
        generateParameters(this.kind, this.type as FunctionType, this.scope),
      ),
