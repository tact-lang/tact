import { AstTrait, AstExpression } from "../../../src/ast/ast";
...
import { Type, TypeGen, makeFunctionTy, UtilType } from "../types";
...
export class Trait extends GenerativeEntity<AstTrait> {
  ...
  constructor(programScope: Scope, params: Partial<TraitParameters> = {}) {
    ...
    const { fieldNum = 1, methodDeclarationsNum = 1, constantNum = 1 } = params;
  ...
  private prepareDeclarationTypes() {
    this.fieldDeclarations = Array.from({ length: this.fieldNum }).map((_) => {
    ...
    Array.from({ length: this.constantNum }).forEach((_) => {
      ...
      else this.constantDeclarations.push(new ConstantDecl(this.scope, ty));
    });
  }
