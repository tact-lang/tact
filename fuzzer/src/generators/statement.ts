...
import { Type, StdlibType, UtilType, tyToAstType } from "../types";
import { Scope, ScopeItemKind } from "../scope";
...
export class AssignStatement extends GenerativeEntity<AstStatement> {
  ...
  generate(): fc.Arbitrary<AstStatement> {
    ...
    if (this.rhsTy.kind === "stdlib" && this.rhsTy.type === StdlibType.Int) {
      assigns.push(
  ...
  generate(): fc.Arbitrary<AstStatementCondition> {
    ...
    return fc.record<AstStatementCondition>({
      ...
      falseStatements: this.falseStmts
        ...
        : fc.constant(undefined),
      // elseif: this.elseif ? this.elseif : fc.constant(null), // TODO: check how if else works
      id: fc.constant(this.idx),
      loc: fc.constant(dummySrcInfo),
    });
  }
...
export class Statement extends GenerativeEntity<AstStatement> {
  ...
  generate(): fc.Arbitrary<AstStatement> {
    ...
    const loopStmt = randomBool() ? this.makeWhileUntil() : this.makeRepeat();
  ...
  private collectLocalMapIds(entryKinds: ScopeItemKind[]): AstId[] {
    return this.parentScope
      ...
      .map(([mapName, _]: [string, Type]) => generateAstIdFromName(mapName));
  }
  ...
  private collectFieldMapIds(entryKinds: ScopeItemKind[]): AstFieldAccess[] {
    return this.parentScope
      .getEntriesRecursive(...entryKinds)
      .filter(([_, mapTy]: [string, Type]) => mapTy.kind === "map")
      .map(([mapName, _]: [string, Type]) => generateFieldAccess(mapName));
  }
  ...
  private makeForEach(): fc.Arbitrary<AstStatement> | undefined {
    ...
    return new ForeachStatement(map, keyVarName, valueVarName, body).generate();
  }

  /**
   * Generates conditional statements.
   * @param allowElseif Allow creating additional AstStatementCondition in else-if blocks.
   */
  private makeCondition(
    allowElseif: boolean = true,
  ): fc.Arbitrary<AstStatementCondition> | undefined {
    if (this.recursionLevel >= this.nestedBlocksNum) {
      return undefined;
    }
    const trueStatements = this.makeStmtsBlock();
    const falseStatements = randomBool() ? undefined : this.makeStmtsBlock();
    const elseif =
      allowElseif && falseStatements === undefined
        ? this.makeCondition(false)
        : undefined;
  ...
  private makeCall(): fc.Arbitrary<AstStatement> | undefined {
    ...
    if (this.parentScope.definedIn("method", "contract") && randomBool()) {
      // Call a method
      const methodEntries = this.parentScope.getEntriesRecursive("methodDef");
      ...
      // Call a function
      const funEntries = this.parentScope.getEntriesRecursive("functionDef");
  ...
  private makeStmtsBlock(blockScope?: Scope): fc.Arbitrary<AstStatement>[] {
    const scope =
      blockScope === undefined
        ? new Scope("block", this.parentScope)
        : blockScope;
