...
export function generateNull(): fc.Arbitrary<AstExpression> {
...
export function generateFunctionCallArgs(
  ...
  if (funTy.kind !== "function") {
    throw new Error(`Incorrect type for function: ${JSONbig.stringify(funTy)}`);
  }
 ...
 */
export class FieldAccess extends GenerativeEntity<AstExpression> {
  constructor(
    type: Type,
    fieldName: string,
    private src?: AstId,
 ...
 */
export class MethodCall extends GenerativeEntity<AstExpression> {
  constructor(
    type: Type,
    name: string,
 ...
 */
export class StaticCall extends GenerativeEntity<AstExpression> {
  constructor(
    type: Type,
    name: string,
    private args?: fc.Arbitrary<AstExpression>[],
...
> {
  ...
  generate(): fc.Arbitrary<AstExpression> | undefined {
  ...
  private findStructsWithMatchingFields(): Map<string, [Type, string[]]> {
    return this.parentScope
      ...
      .reduce((acc, struct) => {
        ...
        const matchingFieldNames = struct.type.fields.reduce((acc, field) => {
        ...
        if (matchingFieldNames.length > 0) {
          acc.set(struct.name?.text!, [struct.type, matchingFieldNames]);
        }
        return acc;
  ...
  private createStructInstance(
    ...
    const chosenStructName = randomElement(Array.from(structEntries.keys()));
    const structType = structEntries.get(chosenStructName)![0];
    const initExpr = new Expression(this.parentScope, structType).generate();
  ...
  private createFieldAccessExpression(
    ...
    const chosenStructName = randomElement(Array.from(structVarNames.keys()));
    const [_, varNames] = structVarNames.get(chosenStructName)!;
    const varName = randomElement(varNames);
    const fieldName = randomElement(structEntries.get(chosenStructName)![1]);
...
export class Expression extends GenerativeEntity<AstExpression> {
  ...
  private makeConstantUse(ty: Type): fc.Arbitrary<AstExpression> | undefined {
    ...
    if (ty.kind === "map" || ty.kind === "struct" || ty.kind === "message") {
      return undefined;
    }
    ...
    const arbs = scope.definedIn("contract", "method")
      ...
      : constantNames.map((name) => fc.constant(generateAstIdFromName(name)));
   ...
   */
  private makeFieldUse(ty: Type): fc.Arbitrary<AstExpression> | undefined {
    if (
    ...
    const arbs = fieldNames.map((name) => new FieldAccess(ty, name).generate());
    return arbs.length > 0 ? fc.oneof(...arbs) : undefined;
  }
   ...
   */
  private makeLocalVarUse(ty: Type): fc.Arbitrary<AstExpression> | undefined {
    if (
      this.compileTimeEval ||
   ...
   */
  private makeFunCall(returnTy: Type): fc.Arbitrary<AstExpression> | undefined {
    ...
    const stdlibArbs = [
      ...
      ...this.parentScope
        ...
        .reduce((acc, [mapName, mapTy]) => {
          if (mapTy.kind === "map" && tyEq(mapTy.type.value, returnTy)) {
            const opCall = new MethodCall(
              returnTy,
              "get",
              { kind: "id", id: nextId(), text: mapName, loc: dummySrcInfo },
              [
                fc.constantFrom(generateThisID()),
                new Expression(this.parentScope, mapTy.type.key).generate(),
              ],
            ).generate();
            acc.push(opCall);
          }
  ...
  private generateRequireCall(): fc.Arbitrary<AstExpression> {
    ...
    return new StaticCall({ kind: "util", type: UtilType.Unit }, "require", [
      condition,
      error,
    ]).generate();
  }
