 ...
 */
const StdlibTypeCache = new Map<StdlibType, [AstTypeDecl, AstType, TypeRef]>();
...
export function tyToAstType(ty: Type, isBounced = false): AstType {
  let generateAstTypeId = (text: string) =>
    <AstTypeId>{
      kind: "type_id",
      text,
      id: nextId(),
...
export function getStdlibTypes(): AstTypeDecl[] {
  return [...Object.values(StdlibType)].map((type) => make.PrimitiveType(type));
}
...
export class TypeGen {
   ...
   */
  public generateFun(minLength = 1, maxLength = 3): fc.Arbitrary<FunctionType> {
  ...
  public generateMethod(): fc.Arbitrary<FunctionType> {
    return this.generateFun().map((funType) => ({
      kind: "function",
      signature: [{ kind: "util", type: UtilType.This }, ...funType.signature],
    }));
  }
  ...
  public generateStruct(isMessage: boolean): fc.Arbitrary<Type> {
    ...
    const structName = createSample(
      generateName(this.scope, kind, /*shadowing=*/ true, /*isType=*/ true),
    );
