...
import { VirtualFileSystem } from "../../src/vfs/VirtualFileSystem";
...
import { AstId, AstNode } from "../../src/ast/ast";
...
export function createProperty<Ts extends [unknown, ...unknown[]]>(
  ...
  const enhancedPredicate = (...args: Ts): boolean | void => {
    args.forEach((arg) => GlobalContext.printSample(arg as AstNode));
...
function makeParams<T>(numRuns: number | undefined): fc.Parameters<T> {
  return {
    numRuns: numRuns ? numRuns : GlobalContext.config.numRuns,
    seed: GlobalContext.config.seed,
    reporter(out) {
      if (out.failed) {
        if (
          out.counterexample !== null &&
          out.counterexample !== undefined &&
          out.error !== null &&
          out.error !== undefined
        ) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          let generated: any = out.counterexample;
          if (!generated.kind && generated[0] && generated[0].kind) {
