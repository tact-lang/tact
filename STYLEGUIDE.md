...
- Don't explicitly break type system.
    - Don't use `as`. It was meant for gradually typing legacy code, not for production use in TS-native projects.
        - Workarounds for bugs in compiler will sometimes require an `as`. In this case the workaround should be small and generic, separated into its own file, and annotated with reference to TypeScript issue.
        - `as const` is not a type cast, and is not considered an `as`.
        - Often `x as T` actually was meant to be `const y: T = x;` or `x satisfies T`.
    - Don't use `any`. Its actual meaning is "completely disregard type errors here". Prefer `unknown` or `never`.
        - Sometimes has to be used as a workaround too.
    - Don't use guard types `x is T`. These are just `as` in disguise.
        - Might be used in very simple cases, such as ADT boilerplate.
    - Don't use overloading (several functions with same name and different signatures).
...
- Don't use JS object "features"
    ...
    - _Don't use optional fields_. In JS there is a distinction between field that is not defined and a field that is `undefined` (sic). `'a' in {} === false`, `'a' in { a: undefined } === true`. TypeScript doesn't handle this properly in its type system.
...
- Don't use JS function "features"
    - Don't use optional parameters. TypeScript has a quirk that allows passing function with less arguments to parameter that requires more. Eventually this leads to passing unexpected values to optional parameters. Prefer decomposing the function into two, where one takes full set of parameters, and the other one is a simpler version that takes less.
    - Don't use default parameters. See reasoning above.
    - Don't use `...rest` parameter. TypeScript doesn't have "mapped tuple types", so typing these will be problematic. In most cases passing an array would suffice, and would only take two more characters to type.
