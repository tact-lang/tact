...
export class WriterContext {
    ...
    //
    // New FunC backend functionality
    //

    /**
     * Collects names of the functions the given function calls.
     */
    private collectDependencies(fun: FuncAstFunctionDefinition): Set<string> {
        const depends = new Set<string>();
        forEachExpression(fun, (expr) => {
            // XXX: It doesn't save receivers. But should it?
            if (
                expr.kind === "expression_fun_call" &&
                expr.object.kind === "plain_id" &&
                !funcBuiltinFunctions.includes(expr.object.value)
            ) {
    ...
    /**
     * Parses the FunC function definition saving it to this context.
     *
     * @param src FunC source that includes exactly one function definition.
     * @throws If the given source cannot be parsed as a simple function/asm function definition.
     */
    public parse<
    ...
    >(
        src: string,
        {
            context = "",
            comment = "",
        }: Partial<{ context: string; comment: string }>,
    ): T | never {
        const mod = parse(src) as FuncAstModule;
        if (
            mod.items.length !== 1 ||
            !["function_definition", "asm_function_definition"].includes(
                mod.items[0]!.kind,
            )
        ) {
            console.error();
        ...
        const fundef = mod.items[0]! as T;
        const name = fundef.name.value;
        this.throwIfDefined(name);
        const pp = new FuncPrettyPrinter();
        const code: Body =
            fundef.kind === "function_definition"
                ? {
                      kind: "generic",
                      code: pp.prettyPrintStatements(fundef.statements),
                  }
                : {
                      kind: "asm",
                      shuffle: "",
                      code: pp.prettyPrintAsmStrings(fundef.asmStrings),
                  };
        const depends =
            fundef.kind === "function_definition"
                ? this.collectDependencies(fundef)
                : new Set<string>();
        const signature = pp.prettyPrintFunctionSignature(
            fundef.returnTy,
            fundef.name,
            fundef.parameters,
            fundef.attributes,
        );
        const flags = fundef.attributes.reduce((set, attr) => {
            if (
                attr.kind === "inline" ||
                attr.kind === "impure" ||
                attr.kind === "inline_ref"
            ) {
                set.add(attr.kind);
            }
            return set;
    ...
    extract(debug: boolean = false) {
        ...
        for (const f of this.functions.values()) {
            const updatedDepends = new Set<string>();
            for (const d of f.depends) {
                if (this.functions.has(d)) {
                    updatedDepends.add(d);
                }
            }
            f.depends = updatedDepends;
        }
    ...
    private throwIfDefined(name: string): void | never {
        if (this.functions.has(name)) {
            throw new Error(`Function "${name}" already defined`);
        }
        if (this.functionsRendering.has(name)) {
            throw new Error(`Function "${name}" already rendering`);
        }
    }
