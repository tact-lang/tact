...
// function getOwner(provider: ContractProvider): Promise<Address> {
//     return provider.get("getOwner", []);
// }
    ...
    describe("NFT Item Contract", () => {
        ...
        describe("Transfer Ownership Tests", () => {
            it("Test ownership assigned", async () => {
                ...
                await step(
                    "Check that newOwner equals notOwner.address",
                    () => {
        ...
        describe("NOT INITIALIZED TESTS", () => {
            ...
            beforeEach(async () => {
                itemNFT = blockchain.openContract(
                    await fromInitItem(null, null, owner.address, itemIndex),
                );
    ...
    describe("NFT Collection Contract", () => {
        ...
        describe("ROYALTY TESTS", () => {
            ...
            it("test royalty getter", async () => {
                const currRoyaltyParams =
                    await collectionNFT.getRoyaltyParams();
                expect(
                    beginCell()
        ...
        describe("NFT DEPLOY TESTS", () => {
            ...
            it("should mint NFTItem correctly", async () => {
                ...
                await step(
                    "Check that nftData.owner equals owner.address",
                    () => {
                        expect(nftData.owner).toEqualAddress(owner.address);
                    },
                );
                await step(
                    "Check that nftData.itemIndex is nextItemIndex",
                    () => {
            ...
            it("should not deploy previous nft", async () => {
                let nextItemIndex: bigint =
                    await collectionNFT.getNextItemIndex();
                for (let i = 0; i < 10; i++) {
            ...
            it("test get nft by itemIndex", async () => {
                ...
                await step(
                    "Check that dataNFT.itemIndex is nextItemIndex",
                    () => {
