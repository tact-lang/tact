...
export const CstNode = (
    children: readonly Cst[],
    type: string = "unknown",
    field: string = "",
    group: string = "",
): CstNode => {
...
export const Constant: Rule = (ctx, b, field) => {
    ...
    r = r && Id(ctx, b2, "name");
    r = r && Constant_optional_16(ctx, b2, "type");
    r = r && Constant_alt_17(ctx, b2, "body");
    if (r && b2.length > 0) {
...
export const ConstantAttribute: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = ConstantAttribute_alt_18(ctx, b2, "name");
    if (r && b2.length > 0) {
...
export const MessageDecl: Rule = (ctx, b, field) => {
    ...
    r = r && MessageDecl_optional_20(ctx, b2, "opcode");
    r = r && TypeId(ctx, b2, "name");
    r = r && consumeString(ctx, b2, "{");
    r = r && structFields(ctx, b2, "fields");
    r = r && consumeString(ctx, b2, "}");
...
export const structFields: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = structFields_optional_21(ctx, b2);
    r = r && structFields_optional_22(ctx, b2);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    if (!r) {
...
export const FieldDecl: Rule = (ctx, b, field) => {
    ...
    r = r && FieldDecl_optional_24(ctx, b2, "expression");
    if (r && b2.length > 0) {
...
export const Contract: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = Contract_star_25(ctx, b2, "attributes");
    r = r && keyword((ctx, b) => consumeString(ctx, b, "contract"))(ctx, b2);
    r = r && Id(ctx, b2, "name");
    r = r && Contract_optional_26(ctx, b2, "parameters");
    r = r && Contract_optional_27(ctx, b2, "traits");
    r = r && consumeString(ctx, b2, "{");
    r = r && Contract_star_28(ctx, b2, "declarations");
    r = r && consumeString(ctx, b2, "}");
    if (r && b2.length > 0) {
...
export const Trait: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = Trait_star_29(ctx, b2, "attributes");
    r = r && keyword((ctx, b) => consumeString(ctx, b, "trait"))(ctx, b2);
    r = r && Id(ctx, b2, "name");
    r = r && Trait_optional_30(ctx, b2, "traits");
    r = r && consumeString(ctx, b2, "{");
    r = r && Trait_star_31(ctx, b2, "declarations");
    r = r && consumeString(ctx, b2, "}");
    if (r && b2.length > 0) {
...
export const FunctionAttribute: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = FunctionAttribute_alt_32(ctx, b2, "name");
    if (r && b2.length > 0) {
...
export const GetAttribute: Rule = (ctx, b, field) => {
    ...
    r = r && GetAttribute_optional_34(ctx, b2, "methodId");
    if (r && b2.length > 0) {
...
export const ReceiverType: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = ReceiverType_alt_35(ctx, b2, "name");
    if (r && b2.length > 0) {
...
export const receiverParam: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = receiverParam_optional_37(ctx, b2);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    if (!r) {
...
export const assembly: Rule = (ctx, b, field) => {
    ...
    const r = assembly_stringify_38(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const assemblyItem: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = assemblyItem_seq_39(ctx, b2);
    r = r || ((ctx.p = p), Comment(ctx, b2));
    r = r || ((ctx.p = p), assemblyItem_seq_41(ctx, b2));
    r = r || ((ctx.p = p), assemblyItem_plus_45(ctx, b2));
...
export const TypeOptional: Rule = (ctx, b, field) => {
    ...
    r = r && TypeOptional_star_48(ctx, b2, "optionals");
    if (r && b2.length > 0) {
...
export const TypeGeneric: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = TypeGeneric_alt_49(ctx, b2, "name");
    r = r && consumeString(ctx, b2, "<");
...
export const TypeId: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = TypeId_lex_53(ctx, b2, "name");
    if (r && b2.length > 0) {
...
export const typeArgs: Rule = (ctx, b, field) => {
    return generic($type)(ctx, b, field);
};
export const generic: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
        let r = consumeString(ctx, b2, "<");
        r = r && generic_optional_54(T)(ctx, b2);
        r = r && consumeString(ctx, b2, ">");
        if (r && b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
...
export const StatementLet: Rule = (ctx, b, field) => {
    ...
    r = r && Id(ctx, b2, "name");
    r = r && StatementLet_optional_56(ctx, b2, "type");
    r = r && consumeString(ctx, b2, "=");
...
export const StatementDestruct: Rule = (ctx, b, field) => {
    ...
    r =
        r &&
        inter(destructItem, (ctx, b) => consumeString(ctx, b, ","))(
            ctx,
            b2,
            "fields",
        );
...
export const StatementReturn: Rule = (ctx, b, field) => {
    ...
    r = r && StatementReturn_optional_57(ctx, b2, "expression");
    r = r && semicolon(ctx, b2);
    if (r && b2.length > 0) {
...
export const StatementAssign: Rule = (ctx, b, field) => {
    ...
    r = r && StatementAssign_alt_58(ctx, b2, "operator");
...
export const StatementCondition: Rule = (ctx, b, field) => {
    ...
    r = r && StatementCondition_optional_61(ctx, b2, "falseBranch");
    if (r && b2.length > 0) {
...
export const StatementTry: Rule = (ctx, b, field) => {
    ...
    r = r && statements(ctx, b2, "body");
    r = r && StatementTry_optional_63(ctx, b2, "handler");
    if (r && b2.length > 0) {
...
export const semicolon: Rule = (ctx, b, field) => {
    ...
    const p = ctx.p;
    let r = consumeString(ctx, b2, ";");
    r = r || ((ctx.p = p), semicolon_lookpos_64(ctx, b2));
...
export const optionalRest: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = optionalRest_seq_65(ctx, b2);
...
export const Conditional: Rule = (ctx, b, field) => {
    ...
    r = r && Conditional_optional_67(ctx, b2, "tail");
    if (r && b2.length > 0) {
...
export const and: Rule = (ctx, b, field) => {
    return Binary(bitwiseOr, (ctx, b) => consumeString(ctx, b, "&&"))(
        ctx,
        b,
        field,
    );
...
export const bitwiseOr: Rule = (ctx, b, field) => {
    return Binary(bitwiseXor, (ctx, b) => consumeString(ctx, b, "|"))(
        ctx,
        b,
        field,
    );
...
export const bitwiseXor: Rule = (ctx, b, field) => {
    return Binary(bitwiseAnd, (ctx, b) => consumeString(ctx, b, "^"))(
        ctx,
        b,
        field,
    );
...
export const bitwiseAnd: Rule = (ctx, b, field) => {
    return Binary(equality, (ctx, b) => consumeString(ctx, b, "&"))(
        ctx,
        b,
        field,
    );
...
export const equality: Rule = (ctx, b, field) => {
    return Binary(compare, equality_alt_68)(ctx, b, field);
};
export const compare: Rule = (ctx, b, field) => {
    return Binary(bitwiseShift, compare_alt_69)(ctx, b, field);
};
export const bitwiseShift: Rule = (ctx, b, field) => {
    return Binary(add, bitwiseShift_alt_70)(ctx, b, field);
};
export const add: Rule = (ctx, b, field) => {
    return Binary(mul, add_alt_71)(ctx, b, field);
};
export const mul: Rule = (ctx, b, field) => {
    return Binary(Unary, (ctx, b) =>
        consumeClass(ctx, b, (c) => c === "*" || c === "/" || c === "%"),
    )(ctx, b, field);
};
export const Unary: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = Unary_star_72(ctx, b2, "prefixes");
...
export const Suffix: Rule = (ctx, b, field) => {
    ...
    r = r && Suffix_star_73(ctx, b2, "suffixes");
    if (r && b2.length > 0) {
...
export const primary: Rule = (ctx, b, field) => {
    ...
    const p = ctx.p;
    let r = Parens(ctx, b2);
    r = r || ((ctx.p = p), MapLiteral(ctx, b2));
    r = r || ((ctx.p = p), SetLiteral(ctx, b2));
...
export const StructInstanceFields: Rule = (ctx, b, field) => {
    ...
    const p = ctx.p;
    let r = consumeString(ctx, b2, "{");
    r = r && StructInstanceFields_optional_74(ctx, b2, "fields");
    r = r && consumeString(ctx, b2, "}");
    if (r && b2.length > 0) {
...
export const MapLiteral: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = keyword((ctx, b) => consumeString(ctx, b, "map"))(ctx, b2);
    r = r && typeArgs(ctx, b2, "typeArgs");
    r = r && consumeString(ctx, b2, "{");
    r = r && MapLiteral_optional_75(ctx, b2, "fields");
    r = r && consumeString(ctx, b2, "}");
    if (r && b2.length > 0) {
        b.push(CstNode(b2, "MapLiteral", field ?? ""));
    }
    if (!r) {
        ctx.p = p;
    }
...
export const mapField: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = expression(ctx, b2, "key");
    r = r && consumeString(ctx, b2, ":");
    r = r && expression(ctx, b2, "value");
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    if (!r) {
...
export const SetLiteral: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "set");
    r = r && typeArgs(ctx, b2, "typeArgs");
    r = r && consumeString(ctx, b2, "{");
    r = r && SetLiteral_optional_76(ctx, b2, "fields");
    r = r && consumeString(ctx, b2, "}");
    if (r && b2.length > 0) {
        b.push(CstNode(b2, "SetLiteral", field ?? ""));
    }
    if (!r) {
        ctx.p = p;
    }
...
export const StructFieldInitializer: Rule = (ctx, b, field) => {
    ...
    const p = ctx.p;
    let r = Id(ctx, b2, "name");
    r = r && StructFieldInitializer_optional_78(ctx, b2, "init");
    if (r && b2.length > 0) {
...
export const ParameterList: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        ...
        const p = ctx.p;
        let r = consumeString(ctx, b2, "(");
        r = r && ParameterList_optional_79(T)(ctx, b2);
        r = r && consumeString(ctx, b2, ")");
        if (r && b2.length > 0) {
...
export const commaList: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        ...
        r = r && commaList_optional_80(T)(ctx, b2);
        if (r && b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
        if (!r) {
...
export const IntegerLiteral: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = IntegerLiteral_alt_81(ctx, b2, "value");
    if (r && b2.length > 0) {
...
export const IntegerLiteralDec: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = IntegerLiteralDec_lex_82(ctx, b2, "digits");
    if (r && b2.length > 0) {
...
export const IntegerLiteralHex: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = IntegerLiteralHex_lex_84(ctx, b2, "digits");
    if (r && b2.length > 0) {
...
export const IntegerLiteralBin: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = IntegerLiteralBin_lex_86(ctx, b2, "digits");
    if (r && b2.length > 0) {
...
export const IntegerLiteralOct: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = IntegerLiteralOct_lex_88(ctx, b2, "digits");
    if (r && b2.length > 0) {
...
export const underscored: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const p = ctx.p;
        const r = underscored_seq_92(T)(ctx, []);
        if (r) {
...
export const FuncId: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = FuncId_optional_98(ctx, b2, "accessor");
    r = r && FuncId_stringify_103(ctx, b2, "id");
    if (r && b2.length > 0) {
...
export const BoolLiteral: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = BoolLiteral_alt_104(ctx, b2, "value");
    r = r && BoolLiteral_lookneg_105(ctx, b2);
    if (r && b2.length > 0) {
...
export const StringLiteral: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = StringLiteral_lex_111(ctx, b2, "value");
    if (r && b2.length > 0) {
...
export const escapeChar: Rule = (ctx, b, field) => {
    ...
    r = r || ((ctx.p = p), escapeChar_seq_119(ctx, b2));
    r = r || ((ctx.p = p), escapeChar_seq_122(ctx, b2));
    r = r || ((ctx.p = p), escapeChar_seq_125(ctx, b2));
...
export const keyword: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        ...
        const r = keyword_seq_127(T)(newCtx, b2);
        if (r) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
        ctx.p = newCtx.p;
...
export const reservedWord: Rule = (ctx, b, field) => {
    return keyword(reservedWord_alt_128)(ctx, b, field);
};
export const space: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const r = space_alt_132(ctx, b2);
    if (r) {
        let p = ctx.p;
        while (((p = ctx.p), space_alt_132(ctx, b2))) {}
        ctx.p = p;
    }
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
...
export const multiLineComment: Rule = (ctx, b, field) => {
    ...
    const p = ctx.p;
    let r = consumeString(ctx, b2, "/*");
    r = r && multiLineComment_stringify_136(ctx, b2);
    r = r && consumeString(ctx, b2, "*/");
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const JustImports: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = JustImports_star_139(ctx, b2, "imports");
...
export const inter: (A: Rule, B: Rule) => Rule = (A, B) => {
    return (ctx, b, field) => {
        ...
        const p = ctx.p;
        let r = A(ctx, b2, "head");
        r = r && inter_star_142(A, B)(ctx, b2, "tail");
        if (r && b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
        if (!r) {
...
export const Constant_optional_16: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = ascription(ctx, b2);
    r = r || ((ctx.p = p), true);
...
export const Constant_alt_17: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const ConstantAttribute_alt_18: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    ...
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "override"))(ctx, b2));
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "abstract"))(ctx, b2));
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    return r;
};
export const MessageDecl_seq_19: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "(");
    r = r && expression(ctx, b2);
...
export const MessageDecl_optional_20: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = MessageDecl_seq_19(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const structFields_optional_22: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, ";");
    r = r || ((ctx.p = p), true);
...
export const FieldDecl_seq_23: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "=");
    r = r && expression(ctx, b2);
...
export const FieldDecl_optional_24: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = FieldDecl_seq_23(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const Contract_star_25: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const Contract_optional_27: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const Contract_star_28: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const Trait_optional_30: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = inheritedTraits(ctx, b2);
    r = r || ((ctx.p = p), true);
...
export const Trait_star_31: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const FunctionAttribute_alt_32: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    ...
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "mutates"))(ctx, b2));
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "extends"))(ctx, b2));
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "virtual"))(ctx, b2));
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "override"))(ctx, b2));
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "inline"))(ctx, b2));
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "abstract"))(ctx, b2));
...
export const GetAttribute_seq_33: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "(");
    r = r && expression(ctx, b2);
...
export const GetAttribute_optional_34: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = GetAttribute_seq_33(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const ReceiverType_alt_35: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "bounced");
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "receive"))(ctx, b2));
    r =
        r ||
        ((ctx.p = p),
        keyword((ctx, b) => consumeString(ctx, b, "external"))(ctx, b2));
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    return r;
};
export const receiverParam_alt_36: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const receiverParam_optional_37: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = receiverParam_alt_36(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const assembly_stringify_38: Rule = (ctx, b, field) => {
    const p = ctx.p;
...
export const assemblyItem_star_40: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const assemblyItem_seq_41: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, '"');
    r = r && assemblyItem_star_40(ctx, b2);
    r = r && consumeString(ctx, b2, '"');
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const assemblyItem_alt_42: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const assemblyItem_lookneg_43: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = assemblyItem_alt_42(ctx, b);
    ctx.p = p;
    return !r;
};
export const assemblyItem_seq_44: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = assemblyItem_lookneg_43(ctx, b2);
...
export const assemblyItem_plus_45: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const r = assemblyItem_seq_44(ctx, b2);
    if (r) {
        let p = ctx.p;
        while (((p = ctx.p), assemblyItem_seq_44(ctx, b2))) {}
        ctx.p = p;
    }
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
...
export const TypeAs_seq_46: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const TypeAs_star_47: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
    while (((p = ctx.p), TypeAs_seq_46(ctx, b2))) {}
    ctx.p = p;
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const TypeOptional_star_48: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const TypeGeneric_alt_49: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const TypeId_star_50: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
    while (
        ((p = ctx.p),
...
export const TypeId_seq_51: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeClass(ctx, b2, (c) => c >= "A" && c <= "Z");
    r = r && TypeId_star_50(ctx, b2);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    if (!r) {
...
export const TypeId_stringify_52: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = TypeId_seq_51(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const TypeId_lex_53: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
    };
    const r = TypeId_stringify_52(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const generic_optional_54: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
...
export const statements_star_55: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const StatementLet_optional_56: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = ascription(ctx, b2);
    r = r || ((ctx.p = p), true);
...
export const StatementReturn_optional_57: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const StatementAssign_alt_58: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const StatementCondition_alt_59: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const StatementCondition_seq_60: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = keyword((ctx, b) => consumeString(ctx, b, "else"))(ctx, b2);
    r = r && StatementCondition_alt_59(ctx, b2);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    if (!r) {
...
export const StatementCondition_optional_61: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = StatementCondition_seq_60(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const StatementTry_seq_62: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const StatementTry_optional_63: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = StatementTry_seq_62(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const semicolon_lookpos_64: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = consumeString(ctx, [], "}");
    ctx.p = p;
    return r;
};
export const optionalRest_seq_65: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, ",");
...
export const Conditional_seq_66: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "?");
...
export const Conditional_optional_67: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = Conditional_seq_66(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const equality_alt_68: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "!=");
    r = r || ((ctx.p = p), consumeString(ctx, b2, "=="));
...
export const compare_alt_69: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "<=");
    r = r || ((ctx.p = p), consumeString(ctx, b2, "<"));
...
export const bitwiseShift_alt_70: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "<<");
    r = r || ((ctx.p = p), consumeString(ctx, b2, ">>"));
...
export const add_alt_71: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "+");
    r = r || ((ctx.p = p), consumeString(ctx, b2, "-"));
...
export const Unary_star_72: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
    while (
        ((p = ctx.p),
...
export const Suffix_star_73: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const StructInstanceFields_optional_74: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const MapLiteral_optional_75: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = commaList(mapField)(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const SetLiteral_optional_76: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = commaList(expression)(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const StructFieldInitializer_seq_77: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, ":");
    r = r && expression(ctx, b2);
...
export const StructFieldInitializer_optional_78: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = StructFieldInitializer_seq_77(ctx, b2);
    r = r || ((ctx.p = p), true);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const ParameterList_optional_79: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
        let r = commaList(T)(ctx, b2);
...
export const commaList_optional_80: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
        let r = consumeString(ctx, b2, ",");
...
export const IntegerLiteral_alt_81: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const IntegerLiteralDec_lex_82: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
...
export const IntegerLiteralHex_seq_83: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const IntegerLiteralHex_lex_84: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
    };
    const r = IntegerLiteralHex_seq_83(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const IntegerLiteralBin_seq_85: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "0");
...
export const IntegerLiteralBin_lex_86: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
    };
    const r = IntegerLiteralBin_seq_85(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const IntegerLiteralOct_seq_87: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "0");
...
export const IntegerLiteralOct_lex_88: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
    };
    const r = IntegerLiteralOct_seq_87(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const underscored_optional_89: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
...
export const underscored_seq_90: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
        let r = underscored_optional_89(T)(ctx, b2);
        r = r && T(ctx, b2);
        if (r && b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
...
export const underscored_star_91: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        let p = ctx.p;
        while (((p = ctx.p), underscored_seq_90(T)(ctx, b2))) {}
        ctx.p = p;
        if (b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
...
export const underscored_seq_92: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
        let r = T(ctx, b2);
        r = r && underscored_star_91(T)(ctx, b2);
        if (r && b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
        if (!r) {
...
export const Id_lookneg_93: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = reservedWord(ctx, b);
    ctx.p = p;
    return !r;
};
export const Id_star_94: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const Id_seq_95: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = Id_lookneg_93(ctx, b2);
    r =
        r &&
        consumeClass(
            ctx,
...
export const Id_stringify_96: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = Id_seq_95(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const Id_lex_97: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
    };
    const r = Id_stringify_96(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const FuncId_optional_98: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const FuncId_plus_99: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const r = consumeClass(
        ctx,
        b2,
        (c) => !(c === "`" || c === "\r" || c === "\n"),
    );
...
export const FuncId_seq_100: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "`");
...
export const FuncId_plus_101: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const r = consumeClass(
        ctx,
        b2,
...
export const FuncId_alt_102: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = FuncId_seq_100(ctx, b2);
    r = r || ((ctx.p = p), FuncId_plus_101(ctx, b2));
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    return r;
};
export const FuncId_stringify_103: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = FuncId_alt_102(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const BoolLiteral_alt_104: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const BoolLiteral_lookneg_105: Rule = (ctx, b, field) => {
    const p = ctx.p;
...
export const StringLiteral_alt_107: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeClass(ctx, b2, (c) => !(c === '"' || c === "\\"));
    r = r || ((ctx.p = p), StringLiteral_seq_106(ctx, b2));
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    return r;
};
export const StringLiteral_star_108: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
    while (((p = ctx.p), StringLiteral_alt_107(ctx, b2))) {}
    ctx.p = p;
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const StringLiteral_stringify_109: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = StringLiteral_star_108(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const StringLiteral_seq_110: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, '"');
    r = r && StringLiteral_stringify_109(ctx, b2);
    r = r && consumeString(ctx, b2, '"');
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const StringLiteral_lex_111: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
    };
    const r = StringLiteral_seq_110(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const escapeChar_optional_113: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = hexDigit(ctx, b2);
    r = r || ((ctx.p = p), true);
...
export const escapeChar_optional_114: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = hexDigit(ctx, b2);
    r = r || ((ctx.p = p), true);
...
export const escapeChar_optional_115: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = hexDigit(ctx, b2);
    r = r || ((ctx.p = p), true);
...
export const escapeChar_optional_116: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = hexDigit(ctx, b2);
    r = r || ((ctx.p = p), true);
...
export const escapeChar_seq_117: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = hexDigit(ctx, b2);
    r = r && escapeChar_optional_112(ctx, b2);
    r = r && escapeChar_optional_113(ctx, b2);
...
export const escapeChar_stringify_118: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = escapeChar_seq_117(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const escapeChar_seq_119: Rule = (ctx, b, field) => {
    ...
    r = r && escapeChar_stringify_118(ctx, b2);
    r = r && consumeString(ctx, b2, "}");
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const escapeChar_seq_120: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = hexDigit(ctx, b2);
    r = r && hexDigit(ctx, b2);
...
export const escapeChar_stringify_121: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = escapeChar_seq_120(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const escapeChar_seq_122: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "u");
    r = r && escapeChar_stringify_121(ctx, b2);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    if (!r) {
...
export const escapeChar_seq_123: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = hexDigit(ctx, b2);
    r = r && hexDigit(ctx, b2);
...
export const escapeChar_stringify_124: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = escapeChar_seq_123(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const escapeChar_seq_125: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = consumeString(ctx, b2, "x");
    r = r && escapeChar_stringify_124(ctx, b2);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    if (!r) {
...
export const keyword_lookneg_126: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const p = ctx.p;
        const r = idPart(ctx, b);
        ctx.p = p;
...
export const keyword_seq_127: (T: Rule) => Rule = (T) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        const p = ctx.p;
        let r = T(ctx, b2);
        r = r && keyword_lookneg_126(T)(ctx, b2);
        if (r && b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
        if (!r) {
...
export const reservedWord_alt_128: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
...
export const space_plus_129: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const r = consumeClass(
        ctx,
        b2,
...
export const space_stringify_130: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = space_plus_129(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const space_lex_131: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const newCtx = {
        ...ctx,
        space: undefined,
    };
    const r = space_stringify_130(newCtx, b2);
    if (r) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
    ctx.p = newCtx.p;
...
export const space_alt_132: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = space_lex_131(ctx, b2);
    r = r || ((ctx.p = p), Comment(ctx, b2));
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const multiLineComment_lookneg_133: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = consumeString(ctx, b, "*/");
    ctx.p = p;
    return !r;
};
export const multiLineComment_seq_134: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    const p = ctx.p;
    let r = multiLineComment_lookneg_133(ctx, b2);
    r = r && consumeAny(ctx, b2);
    if (r && b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const multiLineComment_star_135: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
    while (((p = ctx.p), multiLineComment_seq_134(ctx, b2))) {}
    ctx.p = p;
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
    }
...
export const multiLineComment_stringify_136: Rule = (ctx, b, field) => {
    const p = ctx.p;
    const r = multiLineComment_star_135(ctx, []);
    if (r) {
        const text = ctx.s.substring(p, ctx.p);
        b.push(CstLeaf(text));
    }
...
export const singleLineComment_star_137: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
    while (
        ((p = ctx.p), consumeClass(ctx, b2, (c) => !(c === "\r" || c === "\n")))
    ) {}
    ctx.p = p;
    if (b2.length > 0) {
        b.push(CstNode(b2, field ?? "", field ?? ""));
...
export const singleLineComment_stringify_138: Rule = (ctx, b, field) => {
    const p = ctx.p;
...
export const JustImports_star_139: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
    while (((p = ctx.p), Import(ctx, b2))) {}
    ctx.p = p;
...
export const JustImports_star_140: Rule = (ctx, b, field) => {
    const b2: Builder = [];
    let p = ctx.p;
...
export const inter_star_142: (A: Rule, B: Rule) => Rule = (A, B) => {
    return (ctx, b, field) => {
        const b2: Builder = [];
        let p = ctx.p;
        while (((p = ctx.p), inter_seq_141(A, B)(ctx, b2))) {}
        ctx.p = p;
        if (b2.length > 0) {
            b.push(CstNode(b2, field ?? "", field ?? ""));
        }
