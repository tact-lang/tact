    ...
    it('should implement key deletion correctly', async () => {
        ...
        try {
            ...
            for (let k of keys) {
                ...
                // Clear keys
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelIntMap1', key: k });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelIntMap2', key: k });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelIntMap3', key: k });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelIntMap4', key: k });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelIntMap5', key: k });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelIntMap6', key: keySmall });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelUIntMap7', key: keySmallAbs });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelIntMap8', key: k });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelUIntMap9', key: k });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelAddrMap1', key: addr });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelAddrMap2', key: addr });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelAddrMap3', key: addr });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelAddrMap4', key: addr });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelAddrMap5', key: addr });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelAddrMap6', key: addr });
                await contract.send(treasure, { value: toNano(1) }, { $$type: 'DelAddrMap7', key: addr });
                await system.run();
