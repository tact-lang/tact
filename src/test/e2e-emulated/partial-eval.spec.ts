...
const multiplicativeExpressions = [
    ...
    // to simplify the expression, because if X * -1 overflows/underflows,
    // X * 2 will also.
...
describe("partial-evaluator", () => {
    ...
    it("should correctly simplify partial expressions involving + and -", () => {
        additiveExpressions.forEach(
            pair => testExpression(pair.original, pair.simplified)
        )
    });
    it("should correctly simplify partial expressions involving *", () => {
        multiplicativeExpressions.forEach(
            pair => testExpression(pair.original, pair.simplified)
        )
    });
});
