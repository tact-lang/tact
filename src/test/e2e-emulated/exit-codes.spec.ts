...
describe("exit-codes", () => {
    ...
    it("abi should be correct", async () => {
        ...
        const realFirstExitCode = (await (await parentProvider).get("firstExitCode")).exitCode;
        expect(realFirstExitCode).toBe(firstExitCode);

        const secondExitCode = ExitCodesParent.errors["second"];
        const realSecondExitCode = (await (await parentProvider).get("secondExitCode")).exitCode;
        ...
        const realChildExitCode = (await (await childProvider).get("secondExitCode")).exitCode;
