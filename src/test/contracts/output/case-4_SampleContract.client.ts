...
async function SampleContract_init(opts?: { engine?: ExecutorEngine }) {
    ...
    const __code = '';
    const __system = '';
    ...
    if (res.exitCode !== 0 && res.exitCode !== 1) {
        if (SampleContract_errors[res.exitCode]) {
            throw new ComputeError(SampleContract_errors[res.exitCode].message, res.exitCode, { logs: res.logs });
        } else {
...
export class SampleContract implements Contract {
    ...
    static async init(opts?: { engine?: ExecutorEngine }) {
        return await SampleContract_init(opts);
    }
    
    static async fromInit(opts?: { engine?: ExecutorEngine }) {
        const init = await SampleContract_init(opts);
