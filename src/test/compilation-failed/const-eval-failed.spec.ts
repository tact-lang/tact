...
describe("fail-const-eval", () => {
    ...
    itShouldNotCompile({
        ...
        errorMessage: "Cannot evaluate expression: divisor must be non-zero",
    });
    itShouldNotCompile({
    ...
    itShouldNotCompile({
        ...
        errorMessage:
            "Cannot evaluate expression: integer '115792089237316195423570985008687907853269984665640564039457584007913129639936' does not fit into TVM Int type",
    });
    itShouldNotCompile({
        testName: "const-eval-int-overflow-negative-literal",
        errorMessage:
            "Cannot evaluate expression: integer '-115792089237316195423570985008687907853269984665640564039457584007913129639937' does not fit into TVM Int type",
    });
    itShouldNotCompile({
    ...
    itShouldNotCompile({
        ...
        errorMessage:
            "Cannot evaluate expression: integer '231584178474632390847141970017375815706539969331281128078915168015826259279870' does not fit into TVM Int type",
    });
    itShouldNotCompile({
        testName: "const-eval-int-overflow-mul2",
        errorMessage:
            "Cannot evaluate expression: integer '-231584178474632390847141970017375815706539969331281128078915168015826259279872' does not fit into TVM Int type",
    });
    itShouldNotCompile({
    ...
    itShouldNotCompile({
        ...
        errorMessage: `Cannot evaluate expression: invalid "ton" argument`,
    });
    itShouldNotCompile({
    ...
    itShouldNotCompile({
        ...
        errorMessage:
            "Cannot evaluate expression: integer '-13407807929942597099574024998205846127479365820592393377723561443721764030073546976801874298166903427690031858186486050853753882811946569946433649006084096' does not fit into TVM Int type",
    });
    itShouldNotCompile({
    ...
    itShouldNotCompile({
        ...
        errorMessage:
            "Cannot evaluate expression: invalid address encoding: FQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqB2N",
    });
    ...
    itShouldNotCompile({
        ...
        errorMessage:
            "Cannot evaluate expression: repeat argument must be a number between -2^256 (inclusive) and 2^31 - 1 (inclusive)",
    });
    itShouldNotCompile({
