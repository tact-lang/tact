...
export function storeRenounceOwnership(src: RenounceOwnership) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeUint(472825648, 32);
        b_0.storeUint(src.queryId, 64);
    };
}

export function loadRenounceOwnership(slice: Slice) {
    let sc_0 = slice;
    if (sc_0.loadUint(32) !== 472825648) { throw Error('Invalid prefix'); }
    let _queryId = sc_0.loadUintBig(64);
    return { $$type: 'RenounceOwnership' as const, queryId: _queryId };
}

function loadTupleRenounceOwnership(source: TupleReader) {
    let _queryId = source.readBigNumber();
    return { $$type: 'RenounceOwnership' as const, queryId: _queryId };
}

function storeTupleRenounceOwnership(source: RenounceOwnership) {
    let builder = new TupleBuilder();
    builder.writeNumber(source.queryId);
    return builder.build();
}

function dictValueParserRenounceOwnership(): DictionaryValue<RenounceOwnership> {
    return {
        serialize: (src, buidler) => {
            buidler.storeRef(beginCell().store(storeRenounceOwnership(src)).endCell());
        },
        parse: (src) => {
            return loadRenounceOwnership(src.loadRef().beginParse());
        }
    }
}

 type OwnableTransferableTest_init_args = {
    $$type: 'OwnableTransferableTest_init_args';
    owner: Address;
}

function initOwnableTransferableTest_init_args(src: OwnableTransferableTest_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
...
const OwnableTransferableTest_types: ABIType[] = [
    ...
    {"name":"RenounceOwnership","header":472825648,"fields":[{"name":"queryId","type":{"kind":"simple","type":"uint","optional":false,"format":64}}]},
]

const OwnableTransferableTest_getters: ABIGetter[] = [
    {"name":"owner","arguments":[],"returnType":{"kind":"simple","type":"address","optional":false}},
]

const OwnableTransferableTest_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"text","text":"test"}},
    {"receiver":"internal","message":{"kind":"typed","type":"ChangeOwner"}},
    {"receiver":"internal","message":{"kind":"typed","type":"RenounceOwnership"}},
]

export class OwnableTransferableTest implements Contract {
    
    static async init(owner: Address) {
        return await OwnableTransferableTest_init(owner);
    }
    
    static async fromInit(owner: Address) {
        const init = await OwnableTransferableTest_init(owner);
        const address = contractAddress(0, init);
        return new OwnableTransferableTest(address, init);
    }
    
    static fromAddress(address: Address) {
        return new OwnableTransferableTest(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  OwnableTransferableTest_types,
        getters: OwnableTransferableTest_getters,
        receivers: OwnableTransferableTest_receivers,
        errors: OwnableTransferableTest_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: 'test' | ChangeOwner | RenounceOwnership) {
        ...
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'RenounceOwnership') {
            body = beginCell().store(storeRenounceOwnership(message)).endCell();
        }
