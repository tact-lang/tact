...
export function writeValue(
    ...
    switch (val.kind) {
        ...
        case "map_value": {
            ...
            const res = writeCell(Cell.fromBoc(Buffer.from(val.bocHex, 'hex'))[0]!, wCtx);
            wCtx.used(res);
            return `${res}()`;
        }
    }
...
const writeIdExpr = (f: Ast.Id) => (wCtx: WriterContext): string => {
 ...
 */
const writeBinaryExpr = (f: Ast.OpBinary) => (wCtx: WriterContext): string => {
 ...
 * NOTE: We always wrap expressions in parentheses to avoid operator precedence issues
 */
const writeUnaryExpr = (f: Ast.OpUnary) => (wCtx: WriterContext): string => {
 ...
 */
const writeFieldExpr = (f: Ast.FieldAccess) => (wCtx: WriterContext): string => {
...
const writeStaticCall = (f: Ast.StaticCall) => (wCtx: WriterContext): string => {
...
const writeStructInstance = (f: Ast.StructInstance) => (wCtx: WriterContext): string => {
...
const writeMethodCall = (f: Ast.MethodCall) => (wCtx: WriterContext): string => {
    // Resolve source type
...
const writeInitOf = (f: Ast.InitOf) => (wCtx: WriterContext): string => {
...
const writeCodeOf = (f: Ast.CodeOf) => (wCtx: WriterContext): string => {
...
const writeConditional = (f: Ast.Conditional) => (wCtx: WriterContext): string => {
...
const cannotHappen = (_f: Ast.Expression) => (_wCtx: WriterContext): string => {
    return throwInternalCompilerError("Const evaluator must have handled these cases before");
}
const unsupported = (_f: Ast.Expression) => (_wCtx: WriterContext): string => {
    return throwInternalCompilerError("Set literals are not supported");
}

const writeMapLiteral = (node: Ast.MapLiteral) => (_ctx: WriterContext): string => {
    throwCompilationError("Only constant map literals are supported", node.loc);
...
const writeExpressionAux
    : (f: Ast.Expression) => (wCtx: WriterContext) => string
    = makeVisitor<Ast.Expression>()({
        "id": writeIdExpr,
