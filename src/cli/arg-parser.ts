...
// type GetMatch<T> = [T] extends [Match<infer U>] ? U : never
type Argv = string[]
...
export type GetParserResult<T> = [T] extends [(...args: never[]) => Match<TokenMap<infer U>>] ? U : never;
...
export const ArgParser = (E: CliErrors) => {
    ...
    const boolean = <K extends string>(longName: K, shortName: string | undefined): Token<K, true> => argv => {
        const [head, ...rest] = argv;
    ...
    const string = <K extends string>(longName: K, shortName: string | undefined, argName: string): Token<K, string> => argv => {
            ...
            for (let i = 0; i < iterationLimit; ++i) {
                if (argv.length === 0) {
                    if (!hadErrors) {
                        ...
                        return { kind: 'ok', value: result as Unwrap<TokenMap<T>>, rest: [] };
                    } else {
    ...
    const tokenizer = makeTokenizer<{}>((argv) => {
        const [head, ...rest] = argv;
        return { kind: 'error', errors: [`Unexpected ${head}`], rest };
    });
