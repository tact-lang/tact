...
function processStatements(statements: ASTStatement[], sctx: StatementContext, ctx: CompilerContext): { ctx: CompilerContext, sctx: StatementContext } {
    ...
    for (let s of statements) {
        ...
        if (s.kind === 'statement_let') {
            ...
            sctx = addVariable(s.keyName, { kind: 'ref', name: mapVariable.key, optional: false }, sctx);
            if (sctx.vars[s.valueName]) {
                throwError(`Variable already exists: ${s.valueName}`, s.ref);
            }
            sctx = addVariable(s.valueName, { kind: 'ref', name: mapVariable.value, optional: false }, sctx);
