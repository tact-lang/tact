...
// export type ErrorType = { kind: 'type', type: Type }

export const errorJsonEqual = (left: ErrorJson, right: ErrorJson): boolean => {
    switch (left.kind) {
        case 'link': {
            return left.kind === right.kind && left.text === right.text && srcInfoEqual(left.loc, right.loc);
        }
        case 'at': {
            return left.kind === right.kind && errorJsonEqual(left.body, right.body) && srcInfoEqual(left.loc, right.loc);
        }
        case 'text': {
        ...
        case 'sub': {
            ...
            return left.subst.every((leftChild, index) => {
                ...
                if (typeof rightChild === 'undefined') {
                    throwInternalCompilerError('Impossible: by this moment array lengths must match');
                }
