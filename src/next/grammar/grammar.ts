...
export namespace $ast {
  ...
  export type moduleItem = PrimitiveTypeDecl | $Function | AsmFunction | NativeFunctionDecl | Constant | StructDecl | MessageDecl | UnionDecl | AliasDecl | Contract | Trait;
  ...
  export type storage = IntStorage | CoinsStorage | RemainingStorage | BytesStorage;
  ...
  export type primary = Unit | Tensor | Tuple | Parens | MapLiteral | SetLiteral | StructInstance | StaticCall | IntegerLiteral | BoolLiteral | InitOf | CodeOf | Null | StringLiteral | Id;
...
export const inheritedTraits: $.Parser<$ast.inheritedTraits> = $.right(keyword($.str("with")), commaList(TypeId));
...
export const TypeAs: $.Parser<$ast.TypeAs> = $.loc($.field($.pure("TypeAs"), "$", $.field($.lazy(() => TypeOptional), "type", $.field($.star($.right(keyword($.str("as")), $.lex($.lazy(() => storage)))), "as", $.eps))));
...
export const CoinsStorage: $.Parser<$ast.CoinsStorage> = $.loc($.field($.pure("CoinsStorage"), "$", $.right($.str("coins"), $.eps)));
export const RemainingStorage: $.Parser<$ast.RemainingStorage> = $.loc($.field($.pure("RemainingStorage"), "$", $.right($.str("remaining"), $.eps)));
export const BytesStorage: $.Parser<$ast.BytesStorage> = $.loc($.field($.pure("BytesStorage"), "$", $.right($.str("bytes"), $.field($.stry($.plus($.lazy(() => digit))), "width", $.eps))));
export const storage: $.Parser<$ast.storage> = $.alt(IntStorage, $.alt(CoinsStorage, $.alt(RemainingStorage, BytesStorage)));
...
export const SuffixCall: $.Parser<$ast.SuffixCall> = $.loc($.field($.pure("SuffixCall"), "$", $.field($.opt(typeArgs), "typeArgs", $.field($.lazy(() => parameterList(expression)), "params", $.eps))));
...
export const Unit: $.Parser<$ast.Unit> = $.loc($.field($.pure("Unit"), "$", $.right($.right($.str("("), $.right($.str(")"), $.eps)), $.eps)));
export const Tensor: $.Parser<$ast.Tensor> = $.loc($.field($.pure("Tensor"), "$", $.right($.str("("), $.field(expression, "head", $.field($.plus($.right($.str(","), expression)), "tail", $.right($.opt($.str(",")), $.right($.str(")"), $.eps)))))));
export const Tuple: $.Parser<$ast.Tuple> = $.loc($.field($.pure("Tuple"), "$", $.right($.str("["), $.field($.opt(commaList(expression)), "types", $.right($.str("]"), $.eps)))));
...
export const StructInstance: $.Parser<$ast.StructInstance> = $.loc($.field($.pure("StructInstance"), "$", $.field(TypeId, "type", $.field($.opt(typeArgs), "typeArgs", $.right($.str("{"), $.field($.opt(commaList($.lazy(() => StructFieldInitializer))), "fields", $.right($.str("}"), $.eps)))))));
export const StaticCall: $.Parser<$ast.StaticCall> = $.loc($.field($.pure("StaticCall"), "$", $.field(TypeId, "type", $.field($.opt(typeArgs), "typeArgs", $.right($.str("."), $.field(Id, "name", $.field($.lazy(() => parameterList(expression)), "args", $.eps)))))));
...
export const InitOf: $.Parser<$ast.InitOf> = $.loc($.field($.pure("InitOf"), "$", $.right(keyword($.str("initOf")), $.field(TypeId, "name", $.field($.lazy(() => parameterList(expression)), "params", $.eps)))));
export const CodeOf: $.Parser<$ast.CodeOf> = $.loc($.field($.pure("CodeOf"), "$", $.right($.str("codeOf"), $.field(TypeId, "name", $.eps))));
export const Null: $.Parser<$ast.Null> = $.loc($.field($.pure("Null"), "$", $.right(keyword($.str("null")), $.eps)));
export const primary: $.Parser<$ast.primary> = $.alt(Unit, $.alt(Tensor, $.alt(Tuple, $.alt(Parens, $.alt(MapLiteral, $.alt(SetLiteral, $.alt(StructInstance, $.alt(StaticCall, $.alt(IntegerLiteral, $.alt(BoolLiteral, $.alt(InitOf, $.alt(CodeOf, $.alt(Null, $.alt(StringLiteral, Id))))))))))))));
