...
export const TEViaMember = (via: V.ViaMember): TEViaMember => ({
    kind: "via-member",
    via,
});
...
export const TEMismatch = (tree: MatchTree): TEMismatch => ({
    kind: "mismatch",
    tree,
});
...
export const viaToRange = ({ imports, defLoc: definedAt }: V.ViaUser): Loc => {
    ...
    return throwInternal(
        "Implicit import shadows something. Duplicates in stdlib?",
    );
};

export const ERedefine = (
    kind: string,
    name: string,
    prev: V.Via,
    next: V.ViaUser,
