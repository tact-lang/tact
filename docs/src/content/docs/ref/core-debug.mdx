...
```tact
fun raise(error: String);
```

Unconditionally raises an error with an [exit code](/book/exit-codes) generated from the `error` message, following the same algorithm as the [`require(){:tact}`](#require) function.

The generated exit code is guaranteed to be outside the common 0–255 range reserved for TVM and Tact contract errors, which makes it possible to distinguish exit codes from `raise(){:tact}` and any other [standard exit codes](/book/exit-codes).
