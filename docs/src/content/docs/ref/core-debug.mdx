...
It is also similar to [`throwUnless(){:tact}`](#throwunless), but the exit code is generated from the `error` message instead of being specified in the function arguments.
...
Throws an exception with an error code equal to `code`. Execution of the current context stops (the statements after `throw` won't be executed) and control will be passed to the first [`try...catch{:tact}` block](/book/statements#try-catch) in the call stack. If no `try{:tact}` or `try...catch{:tact}` block exists among caller functions, [TVM](https://docs.ton.org/learn/tvm-instructions/tvm-overview) will terminate the transaction.
...
Similar to [`throw(){:tact}`](#throw), but throws an exception conditionally, when `condition` is equal to `true{:tact}`. Doesn't throw otherwise.
...
```tact {2,7}
fun thisWillTerminate() {
    throwIf(1042, true); // throwing with exit code 1042
}
...
Similar to [`throw(){:tact}`](#throw), but throws an exception conditionally, when `condition` is equal to `false{:tact}`. Doesn't throw otherwise.

It is also similar to [`require(){:tact}`](#require), but doesn't generate an exit code from the error message. Instead, it uses the `code` specified in the function arguments.
...
```tact {2,7}
fun thisWillTerminate() {
    throwUnless(1042, false); // throwing with exit code 1042
}
...
<Badge text="Deprecated since Tact 1.6 (not released yet)" variant="tip" size="medium"/><p/>
