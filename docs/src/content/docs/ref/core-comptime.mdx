...
This page lists all the built-in [global static functions](/book/functions#global-static-functions) that are evaluated when building the Tact project and cannot work with non-constant, run-time data. These functions are commonly referred to as "compile-time functions" or _comptime_ functions for short.
...
A compile-time function that converts the hex-encoded and optionally bit-padded contents of a [`String{:tact}`][p] into a [`Slice{:tact}`][slice] and embeds it into the contract.

Contents are bit-padded if there is an underscore `_` at the very end of the [`String{:tact}`][p]. The padding removes all trailing zeros and the last $1$ bit before them:
...
A compile-time function that concatenates the hexadecimal values of the characters in `str` into one and embeds the resulting [`Int{:tact}`][int] into the contract. It only works for strings occupying up to $32$ bytes, which allows representing up to $32$ [ASCII codes](https://en.wikipedia.org/wiki/ASCII#Control_code_chart) or up to $8$ $4$-byte [Unicode code points](https://en.wikipedia.org/wiki/List_of_Unicode_characters).
