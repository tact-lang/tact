...
This page lists all the built-in [global static functions](/book/functions#global-static-functions) that are evaluated at the time of building the Tact project and cannot work with non-constant runtime data. These functions are commonly referred to as "compile-time functions" or _comptime_ functions for short.
...
A compile-time function that concatenates the hexadecimal values of the characters in `str` into one value and embeds the resulting [`Int{:tact}`][int] into the contract. This function works only for strings occupying up to $32$ bytes, allowing the representation of up to $32$ [ASCII codes](https://en.wikipedia.org/wiki/ASCII#Control_code_chart) or up to $8$ Unicode code points of $4$ bytes each ([list of Unicode characters](https://en.wikipedia.org/wiki/List_of_Unicode_characters)).
