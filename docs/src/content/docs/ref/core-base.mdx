...
Every [contract](/book/contracts) in Tact implicitly [inherits](/book/contracts#traits) the `BaseTrait{:tact}` trait, which contains a number of [member functions](/book/contracts#internal-functions) for any contract and a constant `self.storageReserve{:tact}` aimed at advanced users of Tact.
