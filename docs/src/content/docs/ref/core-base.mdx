...
An alias for calling the [`self.forward(){:tact}`](#self-forward) function with the following arguments:
...
When the [`self.storageReserve{:tact}`](#self-storagereserve) constant is overridden to be greater than 0, before sending a message it also tries to reserve the `self.storageReserve{:tact}` amount of [nanoToncoins][nano] from the remaining balance before making the send in the [`SendRemainingBalance{:tact}`](/book/message-mode#base-modes) ($128$) mode.

If the reservation attempt fails, or by default without the attempt, the message is sent with the [`SendRemainingValue{:tact}`](/book/message-mode#base-modes) ($64$) mode instead.
...
contract Payout with Ownable {
    ...
    bounced(msg: bounced<PayoutOk>) {
        // Reset the completed flag if our message bounces
        self.completed = false;
