...
Calls the native `raw_reserve` function with the specified amount and mode. The `raw_reserve` function creates an output action to reserve a specific amount of [nanoToncoins][nanotoncoin] from the remaining balance of the account.
