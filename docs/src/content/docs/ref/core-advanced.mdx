...
Various niche, dangerous, or unstable features that can produce unexpected results and are meant to be used by more experienced users.
...
An attempt to specify a negative number of `cells`, `bits`, or `seconds` throws an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Calculates and returns the [compute fee][compute-fee] in [nanoToncoin][nanotoncoin] [`Int{:tact}`][int] for a transaction that consumed `gasUsed` amount of [gas][gas]. Uses the prices of the [masterchain][masterchain] if `isMasterchain` is `true{:tact}`, otherwise the prices of the [basechain][basechain]. The current prices are obtained from the [config param 20 for the masterchain and config param 21 for the basechain][param-20-21] of TON Blockchain.

When the `gasUsed` is less than a certain threshold called [`flat_gas_limit`][param-20-21], there is a minimum price to pay based on the value of [`flat_gas_price`][param-20-21]. The less gas is used below this threshold, the higher the minimum price will be. See the example for [`getSimpleComputeFee(){:tact}`](#getsimplecomputefee) to derive that threshold.

An attempt to specify a negative value of `gasUsed` throws an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Similar to [`getComputeFee(){:tact}`](#getcomputefee), but without the [`flat_gas_price`][param-20-21], i.e., without a minimum price to pay if the `gasUsed` is less than a certain threshold called [`flat_gas_limit`][param-20-21]. Calculates and returns only the `gasUsed` multiplied by the current gas price.
...
Note that specifying values of `cells` and `bits` higher than their maximum values as listed in [account state limits (`max_msg_cells` and `max_msg_bits`)](/book/exit-codes#50) will have the same result as specifying the exact limits.

However, regardless of the values of `cells` and `bits`, this function always adds the minimum price based on the value of [`lump_price`][param-24-25]. See the example for [`getSimpleForwardFee(){:tact}`](#getsimpleforwardfee) to derive it. In addition, make sure you take into account the [deduplication of cells with the same hash][deduplication]; for example, the root cell and its data bits don't count towards the forward fee and are covered by the [`lump_price`][param-24-25].

Attempts to specify negative numbers for `cells` or `bits` throw an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Similar to [`getForwardFee(){:tact}`](#getforwardfee), but without the [`lump_price`][param-24-25], i.e., without the minimum price to pay regardless of the amounts of `cells` or `bits`. Calculates and returns only the number of `cells` multiplied by the current cell price plus the number of `bits` multiplied by the current bit price.
...
Sets the [`gas_limit`][param-20-21] to the [`Int{:tact}`][int] `limit` and resets the [`gas_credit`][param-20-21] to $0$. Note that specifying `limit` higher than the maximum allowed value of $2^{63} - 1$ has the same effect as specifying that exact maximum or invoking [`acceptMessage(){:tact}`](#acceptmessage).

Attempting to specify a negative or insufficient value for `limit` causes an exception with [exit code -14](/book/exit-codes#-14): `Out of gas error`.
...
Agrees to buy some [gas][gas] to finish the current transaction by setting the [`gas_limit`][param-20-21] to its maximum allowed value of $2^{63} - 1$ and resetting the [`gas_credit`][param-20-21] to $0$. This action is required to process external messages that bring no value (hence no gas) themselves.
...
contract Timeout {
    ...
    external("timeout") {
        if (now() > self.timeout) {
            acceptMessage(); // start accepting external messages once timeout has expired
        }
    }
}
```
...
Commits the current state of [registers][registers] `c4` (persistent data) and `c5` (actions), so that the current execution is considered "successful" with the saved values even if an exception is thrown later during the compute phase.

Usage example:

```tact {1}
commit();  // now, transaction is considered "successful"
throw(42); // this won't fail the transaction
```
...
Randomizes the pseudorandom number generator with the specified unsigned $256$-bit [`Int{:tact}`][int] `x` by mixing it with the current [seed][seed]. The new seed is the unsigned $256$-bit [`Int{:tact}`][int] value of the [SHA-256][sha-2] hash of the concatenated old seed and `x` in their $32$-byte string's [big-endian](https://en.wikipedia.org/wiki/Endianness) representation.
...
Generates and returns a $256$-bit random number just like [`randomInt(){:tact}`](/ref/core-random#randomint), but doesn't initialize the random generator with [`nativePrepareRandom(){:tact}`](#nativepreparerandom) beforehand.

:::note

  Don't use this function directly; prefer using [`randomInt(){:tact}`](/ref/core-random#randomint) instead.
...
:::note
  ...
  Do not use this function directly; prefer using [`random(){:tact}`](/ref/core-random#random) instead.
...
Attempting to queue more than $255$ messages throws an exception with [exit code 33](/book/exit-codes#33): `Action list is too long`.

:::note

  Prefer using the more common and user-friendly [`send(){:tact}`](/ref/core-common#send) function unless you have complex logic that can't be expressed otherwise.
...
Calls the native `raw_reserve` function with the specified amount and mode. The `raw_reserve` is a function that creates an output action to reserve a specific amount of [nanoToncoins][nanotoncoin] from the remaining balance of the account.
...
The function `raw_reserve` is roughly equivalent to creating an outbound message carrying the specified `amount` of [nanoToncoins][nanotoncoin] (or `b` $-$ `amount` [nanoToncoins][nanotoncoin], where `b` is the remaining balance) to oneself. This ensures that subsequent output actions cannot spend more money than the remainder.

It's possible to use raw [`Int{:tact}`][int] values and manually provide them for the `mode`, but for convenience, there is a set of constants that you may use to easily construct the compound `mode`. Take a look at the following tables for more information on base modes and optional flags.

:::caution

  Currently, `amount` must be a non-negative integer, and `mode` must be in the range $0..31$, inclusive.

  Additionally, attempting to queue more than $255$ reservations in one transaction throws an exception with [exit code 33](/book/exit-codes#33): `Action list is too long`.
...
To create the [`Int{:tact}`][int] value for the `mode` parameter, combine base modes with optional flags using the [bitwise OR](/book/operators#binary-bitwise-or) operation:
...
Attempts to pass a [`Slice{:tact}`][slice] with a layout different from the `StdAddress{:tact}` or to load more data than a given [`Slice{:tact}`][slice] contains throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Attempts to pass a [`Slice{:tact}`][slice] with a layout different from the `VarAddress{:tact}` or to load more data than a given [`Slice{:tact}`][slice] contains throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
