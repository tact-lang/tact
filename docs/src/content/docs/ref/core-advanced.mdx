...
Parameterized version of `forceBasechain`. Checks whether the `address` is in the `workchain`, i.e., its [chain ID](https://docs.ton.org/learn/overviews/addresses#workchain-id) is equal to `workchain` parameter. If it is not, throws an exception with exit code `errorCode`.
