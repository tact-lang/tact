...
Throws an exception with exit code 138: `Not a basechain address` if the `address` is not a basechain address.
...
fun example() {
    ...
    forceBasechain(someBasechainAddress);   // ok
    forceBasechain(someMasterchainAddress); // throws 138
}
```
