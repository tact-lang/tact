...
A simple trait `Deployable{:tact}` that provides a convenient unified mechanism for deployments by implementing a simple receiver for the [Deploy](#deploy) message.

All contracts are deployed by sending them a message. While any message can be used for this purpose, the best practice is to use the special [Deploy](#deploy) message.
