...
Loads all but the first $0 \le l \le 1023$ bits from the [`Slice{:tact}`][slice].
...
Preloads all but the last $0 \le len \le 1023$ bits from the [`Slice{:tact}`][slice].
...
Attempts to load such [`Bool{:tact}`][bool] when the [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Attempts to load such [`Int{:tact}`][int] when the [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Preloads the first $0 \leq \text{len} \leq 1023$ bits from the [`Slice{:tact}`][slice].
...
Converts the [Struct][struct] to a [`Slice{:tact}`][slice] and returns it. An alias to `self.toCell().asSlice(){:tact}`.
...
Attempts to pass a [`Cell{:tact}`][cell] with a layout different from the specified [Struct][struct] or to load more data than the [`Cell{:tact}`][cell] contains throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Attempts to pass a [`Slice{:tact}`][slice] with a layout different from the specified [Struct][struct] or to load more data than the [`Slice{:tact}`][slice] contains throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
