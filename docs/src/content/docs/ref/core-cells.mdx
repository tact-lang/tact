...
[`Cell{:tact}`][cell] is a low-level [primitive][p] that represents data in TON Blockchain. Cells consist of $1023$ bits of data with up to $4$ references to other cells. They are read-only, immutable, and cannot have cyclic references.
...
:::note
  ...
  Every user is recommended to use [Structs][struct] and their [methods](/book/functions#extension-function), such as [`Struct.toCell(){:tact}`](#structtocell) and [`Struct.fromCell(){:tact}`](#structfromcell), instead of manually constructing and parsing cells, because [Structs][struct] and [Messages][message] are the closest to being the [living TL-B schemas of your contracts](/book/cells#cnp-structs).
...
Creates and returns an empty [`Cell{:tact}`][cell] (without data and references). Aliased to `beginCell().endCell(){:tact}`.
...
Creates and returns an empty [`Slice{:tact}`][slice] (without data and references). Aliased to `emptyCell().asSlice(){:tact}`.
...
Computes and returns the [`Int{:tact}`][int] [depth][std-repr] of the [`Cell{:tact}`][cell]. Returns $0$ if the [`Cell{:tact}`][cell] has no references; otherwise, returns $1$ plus the maximum depth of the referenced cells. If `self{:tact}` is [`null{:tact}`][null], returns $0$.
...
If the specified `maxCells` value isn't sufficient to traverse all cells including the starting one, an exception with [exit code 8](/book/exit-codes#8) is thrown: `Cell overflow`.

Attempting to specify a negative value for `maxCells` throws an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Calculates and returns an [`Int{:tact}`][int] value based on the [SHA-256][sha-2] hash of the [standard `Cell{:tact}` representation][std-repr] of the given [`Cell{:tact}`][cell].
...
Converts the Cell to a [`Slice{:tact}`][slice] and returns it. Alias for `self.beginParse(){:tact}`.
...
Attempting to store a negative `value` or provide an insufficient or out-of-bounds `bits` number throws an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Attempting to provide an insufficient or out-of-bounds `bits` number throws an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Alias for [`Builder.storeBool(){:tact}`](#builderstorebool).
...
Appends all data from a [`Builder{:tact}`][builder] `cell` to a copy of the [`Builder{:tact}`][builder]. Returns that copy.
...
Stores a [`Slice{:tact}`][slice] `cell` into a copy of the [`Builder{:tact}`][builder]. Returns that copy.
...
Stores (serializes) an unsigned [`Int{:tact}`][int] `value` in the range from $0$ to $2^{120} - 1$ inclusive into a copy of the [`Builder{:tact}`][builder]. The serialization of `value` consists of a $4$-bit unsigned big-endian integer $l$, which is the smallest integer $l ≥ 0$ such that `value < 2^{8 \cdot l}$, followed by an $8 \cdot l$-bit unsigned big-endian representation of `value`. Returns that copy of the [`Builder{:tact}`][builder].

Attempting to store an out-of-bounds `value` throws an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Alias for [`Builder.storeCoins(){:tact}`](#builderstorecoins).
...
Similar to [`Builder.storeCoins(){:tact}`](#builderstorecoins), but has a different `value` range: from $-2^{119}$ to $2^{119} - 1$ inclusive.
...
Stores (serializes) an unsigned [`Int{:tact}`][int] `value` in the range from $0$ to $2^{248} - 1$ inclusive into a copy of the [`Builder{:tact}`][builder]. The serialization of `value` consists of a $5$-bit unsigned big-endian integer $l$, which is the smallest integer $l ≥ 0$ such that `value < 2^{8 \cdot l}$, followed by an $8 \cdot l$-bit unsigned big-endian representation of `value`. Returns that copy of the [`Builder{:tact}`][builder].
...
Attempts to store an `address` into the [`Builder{:tact}`][builder] `self{:tact}` when `self{:tact}` cannot fit it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
If the `cell` is not `null{:tact}`, stores $1$ as a single bit and then stores the reference `cell` into a copy of the [`Builder{:tact}`][builder]. Returns that copy.

If the `cell` is `null{:tact}`, only stores $0$ as a single bit into a copy of the [`Builder{:tact}`][builder]. Returns that copy.
...
Computes and returns the [`Int{:tact}`][int] [depth][std-repr] of the [`Builder{:tact}`][builder]. Produces $0$ if the [`Builder{:tact}`][builder] has no references stored so far; otherwise, produces $1$ plus the maximum depth of the referenced cells.
...
Preloads and returns an unsigned `l`-bit [`Int{:tact}`][int] from the [`Slice{:tact}`][slice] for $0 ≤ l ≤ 256$. Does not modify the [`Slice{:tact}`][slice].
...
Preloads and returns a signed `l`-bit [`Int{:tact}`][int] from the [`Slice{:tact}`][slice] for $0 ≤ l ≤ 257$. Does not modify the [`Slice{:tact}`][slice].
...
Preloads $0 ≤ l ≤ 1023$ bits from the [`Slice{:tact}`][slice] and returns them as a separate [`Slice{:tact}`][slice]. Does not modify the original [`Slice{:tact}`][slice].
...
```tact
let s: Slice = beginCell().storeInt(42, 7).asSlice();
s.skipBits(5);                   // all but the first 5 bits
...
```tact
let s: Slice = beginCell().storeInt(42, 7).asSlice();
let allButLastFive: Slice = s.skipLastBits(5); // all but the last 5 bits,
...
Alias to [`Slice.loadBool(){:tact}`](#sliceloadbool).
...
Alias to [`Slice.loadCoins(){:tact}`](#sliceloadcoins).
...
Attempts to load such [`Int{:tact}`][int] when the [`Slice{:tact}`][slice] does not contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Attempts to load such [`Address{:tact}`][p] when the [`Slice{:tact}`][slice] does not contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Attempts to load such reference [`Cell{:tact}`][cell] when the [`Slice{:tact}`][slice] does not contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Preloads the next reference from the [`Slice{:tact}`][slice] as a [`Cell{:tact}`][cell]. Does not modify the original [`Slice{:tact}`][slice].

Attempts to preload such a reference [`Cell{:tact}`][cell] when [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Loads a single bit from the [`Slice{:tact}`][slice]: if it is $1$, then the referenced [`Cell{:tact}`][cell] is loaded and returned. If the loaded bit is $0$, then nothing else is loaded and `null{:tact}` is returned.

Attempts to load such a reference [`Cell{:tact}`][cell] when the [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Preloads a single bit from the [`Slice{:tact}`][slice]: if it is $1$, then the referenced [`Cell{:tact}`][cell] is preloaded and returned. If the preloaded bit is $0$, then `null{:tact}` is returned. Does not modify the original [`Slice{:tact}`][slice].

Attempts to preload such a reference [`Cell{:tact}`][cell] when the [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
:::note
  ...
  To reduce gas usage, prefer calling [`Slice.preloadBits(){:tact}`](#slicepreloadbits) over using this function, since the former is more optimized.
...
Attempting to specify an out-of-bounds `len` value throws an exception with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.

Attempting to preload more data than the [`Slice{:tact}`][slice] contains throws an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Similar to [`Cell.computeDataSize(){:tact}`](#cellcomputedatasize) but does not take into account the cell that contains the [`Slice{:tact}`][slice] itself. However, it accounts for its bits and references.
...
`cells` | [`Int{:tact}`][int] | The total number of nested cells, including the starting one
`bits`  | [`Int{:tact}`][int] | The total number of bits in all nested cells, including the starting one
`refs`  | [`Int{:tact}`][int] | The total number of references in all nested cells, including the starting one

If the specified `maxCells` value is not sufficient to traverse all cells **excluding** the starting one, an exception with [exit code 8](/book/exit-codes#8) is thrown: `Cell overflow`.
...
Calculates and returns an [`Int{:tact}`][int] value representing the [SHA-256][sha-2] hash of the [standard `Cell{:tact}` representation][std-repr] of the given [`Slice{:tact}`][slice].
...
Calculates and returns an [`Int{:tact}`][int] value representing the [SHA-256][sha-2] hash of the data bits from the given [`Slice{:tact}`][slice], which should have a number of bits divisible by $8$.
...
Attempting to specify a [`Slice{:tact}`][slice] with a number of bits **not** divisible by $8$ throws an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Casts the [`Slice{:tact}`][slice] into an [`Address{:tact}`][p] in a given `chain` ID and returns it. This is the inverse of [`Address.asSlice(){:tact}`](#addressasslice) and a safe but more gas-expensive version of [`Slice.asAddressUnsafe(){:tact}`](#sliceasaddressunsafe).

Attempting to specify a [`Slice{:tact}`][slice] with an invalid [account ID](https://docs.ton.org/learn/overviews/addresses#account-id) length or an invalid tag prefix (not `0b100`) throws an error with [exit code 136](/book/exit-codes#): `Invalid standard address`.
...
Unsafely casts the [`Slice{:tact}`][slice] to an [`Address{:tact}`][p] and returns it. This function is the inverse of [`Address.asSlice(){:tact}`](#addressasslice).
...
Use this function only if you want to optimize the code for gas and can guarantee in advance that the [`Slice{:tact}`][slice] contains the data of an [`Address{:tact}`][p].
...
Casts the [`Address{:tact}`][p] back to the underlying [`Slice{:tact}`][slice] and returns it. This function is the inverse of [`Slice.asAddressUnsafe(){:tact}`](#sliceasaddressunsafe).
...
Converts the [Struct][struct] to a [`Slice{:tact}`][slice] and returns it. This function is an alias for `self.toCell().asSlice(){:tact}`.
...
Attempting to pass a [`Cell{:tact}`][cell] with a layout different from the specified [Struct][struct], or attempting to load more data than the [`Cell{:tact}`][cell] contains, throws an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Attempting to pass a [`Slice{:tact}`][slice] with a layout different from the specified [Struct][struct], or attempting to load more data than the [`Slice{:tact}`][slice] contains, throws an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Converts the [Message][message] to a [`Slice{:tact}`][slice] and returns it. Alias for `self.toCell().asSlice(){:tact}`.
...
Attempting to pass a [`Cell{:tact}`][cell] with a layout different from the specified [Message][message] or attempting to load more data than a [`Cell{:tact}`][cell] contains will throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Attempting to pass a [`Slice{:tact}`][slice] with a layout different from the specified [Message][message] or attempting to load more data than a [`Slice{:tact}`][slice] contains will throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
