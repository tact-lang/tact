...
:::note
    ...
    If a contract doesn't use [contract parameters](/book/contracts#parameters), the resulting `Cell{:tact}` or `Slice{:tact}` will contain a leading one bit representing the lazy initialization bit.
...
contract Example {
    ...
    get fun asSlice(): Slice {
        return self.toSlice(); // "CS{Cell{000612a11b} bits: 0..24; refs: 0..0}"
    }
}
```
