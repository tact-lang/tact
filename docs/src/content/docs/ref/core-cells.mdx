...
Alias to [`Builder.storeBool(){:tact}`](#builderstorebool).
...
Stores (serializes) an unsigned [`Int{:tact}`][int] `value` in the range from $0$ to $2^{120} − 1$ inclusive into the copy of the [`Builder{:tact}`][builder]. The serialization of `value` consists of a $4$-bit unsigned big-endian integer $l$, which is the smallest integer $l ≥ 0$, such that `value` $< 2^{8 * l}$, followed by an $8 * l$-bit unsigned big-endian representation of `value`. Returns that copy of the [`Builder{:tact}`][builder].
...
Alias to [`Builder.storeCoins(){:tact}`](#builderstorecoins).
...
Stores (serializes) an unsigned [`Int{:tact}`][int] `value` in the range from $0$ to $2^{248} − 1$ inclusive into the copy of the [`Builder{:tact}`][builder]. The serialization of `value` consists of a $5$-bit unsigned big-endian integer $l$, which is the smallest integer $l ≥ 0$, such that `value` $< 2^{8 * l}$, followed by an $8 * l$-bit unsigned big-endian representation of `value`. Returns that copy of the [`Builder{:tact}`][builder].
...
Loads a single bit from the [`Slice{:tact}`][slice]: if it's $1$, then the referenced [`Cell{:tact}`][cell] is loaded and returned. If the loaded bit is $0$, then nothing else is loaded and `null{:tact}` is returned.
...
Preloads a single bit from the [`Slice{:tact}`][slice]: if it's $1$, then the referenced [`Cell{:tact}`][cell] is preloaded and returned. If the preloaded bit is $0$, then `null{:tact}` is returned. Doesn't modify the original [`Slice{:tact}`][slice].
...
Attempts to specify a [`Slice{:tact}`][slice] with number of bits **not** divisible by $8$ throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Casts the [`Slice{:tact}`][slice] to an [`Address{:tact}`][p] in a given `chain` ID and returns it. The inverse of [`Address.asSlice(){:tact}`](#addressasslice) and a safe but more gas-expensive version of [`Slice.asAddressUnsafe(){:tact}`](#sliceasaddressunsafe).
...
:::note
  ...
  Because of the checks performed, this function is more gas-expensive than its unsafe but cheap counterpart: [`Slice.asAddressUnsafe(){:tact}`](#sliceasaddressunsafe).
...
:::caution
  ...
  Otherwise, use a safer but more gas-expensive [`Slice.asAddress(){:tact}`](#sliceasaddress) function.
