...
```tact
extends mutates fun skipUint(self: Slice, l: Int);
```

Extension mutation function for the [`Slice{:tact}`][slice] type.

Skips an unsigned `l`-bit [`Int{:tact}`][int] from the [`Slice{:tact}`][slice], for $0 ≤ l ≤ 256$. Similar to discarding the return value of `Slice.loadUint()`.
...
```tact
extends mutates fun skipInt(self: Slice, l: Int);
```

Extension mutation function for the [`Slice{:tact}`][slice] type.

Skips a signed `l`-bit [`Int{:tact}`][int] from the [`Slice{:tact}`][slice], for $0 ≤ l ≤ 257$. Similar to discarding the return value of `Slice.loadInt()`.
...
Attempts to skip such [`Bool{:tact}`][bool] when the [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
```tact
extends mutates fun skipBit(self: Slice);
```

Extension mutation function for the [`Slice{:tact}`][slice] type.

Alias to `Slice.skipBool()`.
...
Attempts to skip such [`Int{:tact}`][int] when the [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
...
Attempts to skip such reference [`Cell{:tact}`][cell] when the [`Slice{:tact}`][slice] doesn't contain it throw an exception with [exit code 8](/book/exit-codes#8): `Cell overflow`.
