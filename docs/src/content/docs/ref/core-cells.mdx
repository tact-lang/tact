...
Safely casts the [`Slice{:tact}`][slice] to an [`Address{:tact}`][p] and returns it. The inverse of [`Address.asSlice(){:tact}`](#addressasslice).
The safe version of [`Slice.asAddressUnsafe(){:tact}`](#sliceasaddressunsafe).

Usage example:

```tact
let a: Address = myAddress();
let a2: Address = a.asSlice().asAddress();
...
:::caution
  ...
  Otherwise, use a slower but safer [`Slice.asAddress()`](#sliceasaddress) function.
