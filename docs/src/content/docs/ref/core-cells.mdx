...
Loads the next `Maybe^ Cell` reference from the [`Slice{:tact}`][slice] as a [`Cell?{:tact}`][cell].

In other words loads 1 bit and if it is true
loads first ref and return it,
otherwise returns `null`. In both cases modifies the original `Slice`.

Usage examples:

```tact
let cs = msg.asSlice();
let outActions = cs.loadMaybeRef();
...
Preloads the next `Maybe^ Cell` reference from the [`Slice{:tact}`][slice] as a [`Cell?{:tact}`][cell].

In other words loads 1 bit and if it is true
loads first ref and return it,
otherwise returns `null`. In both cases doesn't modify the original `Slice`.

Usage examples:

```tact
let cs = msg.asSlice();
let outActions = cs.preloadMaybeRef();

if (outActions != null) {
    cs.skipBits(1);     // skip Maybe bit
    processActions(cs); // can directly load actions inside
} else {
  return;
}
```
