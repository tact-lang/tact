...
Extension function for the [`Builder{:tact}`][builder]. Alias to [`Builder.storeCoins(){:tact}`](#builderstorecoins).
...
Attempts to store an address into `self` will result in [exit code 8](/book/exit-codes#8): `Cell overflow` if `self` cannot fit the address.
...
Extension mutation function for the [`Slice{:tact}`][slice]. Alias to [`Slice.loadCoins(){:tact}`](#sliceloadcoins).
