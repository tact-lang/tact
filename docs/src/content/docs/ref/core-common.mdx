...
:----------- | :---------------------- | :----------
`bounceable` | [`Bool{:tact}`][bool]   | Can the received message [bounce back](https://docs.ton.org/v3/documentation/smart-contracts/message-management/non-bounceable-messages) or not.
...
The `MessageParameters{:tact}` [struct][struct] is similar to [`SendParameters{:tact}`](/book/send) [struct][struct], but without the `body` and `code` fields.
