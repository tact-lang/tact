...
:::caution
  ...
  Beware that [all message-sending functions](/book/send#message-sending-functions) of Tact can change the _actual_ contract balance, but they **won't** update the value returned by this function.
...
Returns a `Context{:tact}` [Struct](/book/structs-and-messages#structs), consisting of:

Field        | Type                    | Description
:----------- | :---------------------- | :----------
`bounceable` | [`Bool{:tact}`][bool]   | Indicates whether the received message can [bounce back](https://docs.ton.org/v3/documentation/smart-contracts/message-management/non-bounceable-messages) or not.
...
:::note
  ...
  If you only need to know who sent the message, use the [`sender(){:tact}`](#sender) function, as it's less gas-consuming.
...
<Badge text="Available since Tact 1.6.1 (not released yet)" variant="tip" size="medium"/><p/>

```tact
extends fun hasSameBasechainAddress(self: StateInit, sender: Address): Bool;
```

Efficiently compares whether the given address matches the basechain address of the contract.
...
This function provides a gas-optimized implementation compared to a direct address comparison:
...
Computes the smart contract's [`Address{:tact}`][p] in the workchain ID $0$ (basechain) using the [`StateInit{:tact}`](/book/expressions#initof) `s` of the contract. Alias to `contractAddressExt(0, s.code, s.data){:tact}`.
...
Computes the smart contract's [`Address{:tact}`][p] in the `chain` ID using the contract's `code` and initial state `data`. Use the [`initOf{:tact}`](/book/expressions#initof) expression to obtain the initial `code` and initial `data` of a given contract.
...
Attempting to queue more than $255$ messages throws an exception with an [exit code 33](/book/exit-codes#33): `Action list is too long`.
...
[Queues the message](/book/send#outbound-message-processing) `body` to be sent to the outer world for logging and later off-chain analysis. The message does not have a recipient and is more gas-efficient compared to using any other [message-sending functions](/book/send#message-sending-functions) of Tact.
