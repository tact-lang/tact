...
Computes and returns the [rounded up][round-up] result of the division of [`Int{:tact}`][int] `x` by [`Int{:tact}`][int] `y`.
...
If the value in the calculation goes beyond the range from $-2^{256}$ to $2^{256} - 1$ inclusive, or if there's an attempt to divide by `z` equal to $0$, an exception with [exit code 4](/book/exit-codes#4) is thrown: `Integer overflow`.
...
Computes and returns the [rounded down][round-down] result of `(x * y) / 2^z{:tact}`. It is a more gas-efficient equivalent of performing a [bitwise shift right](/book/operators#binary-bitwise-shift-right) on the result of the multiplication of [`Int{:tact}`][int] `x` times [`Int{:tact}`][int] `y`, where [`Int{:tact}`][int] `z` is the right operand of the shift.

If the value in the calculation goes beyond the range from $-2^{256}$ to $2^{256} - 1$ inclusive, an exception with [exit code 4](/book/exit-codes#4) is thrown: `Integer overflow`.
...
Attempts to specify a non-positive `num` value or a `base` less than $1$ throw an error with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Attempts to specify a non-positive `num` value throw an error with [exit code 5](/book/exit-codes#5): `Integer out of expected range`.
...
Computes and returns the [exponentiation](https://en.wikipedia.org/wiki/Exponentiation) involving two numbers: the `base` and the exponent (or _power_) `exp`. The exponent `exp` must be non-negative; otherwise, an error with [exit code 5](/book/exit-codes#5) will be thrown: `Integer out of expected range`.
...
Checks the [Ed25519][ed] `signature` of the $256$-bit unsigned [`Int{:tact}`][int] `hash` using a `public_key`, represented by a $256$-bit unsigned [`Int{:tact}`][int] as well. The signature must contain at least $512$ bits of data, but only the first $512$ bits are used.

Returns `true{:tact}` if the signature is valid; otherwise, returns `false{:tact}`.
...
Checks the [Ed25519][ed] `signature` of the `data` using a `public_key`, similar to [`checkSignature(){:tact}`](#checksignature). If the bit length of `data` is not divisible by $8$, this function throws an error with [exit code 9](/book/exit-codes#9): `Cell underflow`. Verification itself is performed indirectly: on a [SHA-256][sha-2] hash of the `data`.
...
In case `data` is a [`Slice{:tact}`][slice], it must have no more than a single reference per cell, as only the first reference of each nested cell will be taken into account.

This function tries to resolve constant string values at [compile-time](/ref/core-comptime) whenever possible.

Attempting to specify a [`Slice{:tact}`][slice] or [`String{:tact}`][p] with a number of bits **not** divisible by $8$ throws an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
:::tip[Before Tact 1.6]
  ...
  Previously, if a [`String{:tact}`][p] value couldn't be resolved during [compile-time](/ref/core-comptime), the hash was calculated at runtime by the [TVM][tvm] itself. This caused collisions of strings longer than $127$ bytes if their first $127$ bytes were identical.

  This occurred because all [SHA-256][sha-2]-related instructions of the [TVM][tvm] consider only the data bits, ignoring possible references to other cells needed to form longer strings.

  Therefore, in general, and in versions of Tact prior to 1.6, it is preferable to use statically known strings whenever possible. When in doubt, use strings up to $127$ bytes long.
