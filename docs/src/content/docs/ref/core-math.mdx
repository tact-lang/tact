...
Computes and returns the sign of the [`Int{:tact}`][int] value `x`. Produces $1$ if the `x` is positive, $-1$ if the `x` is negative, and $0$ if the `x` is $0$.
...
```tact
fun isqrt(num: Int): Int;
```

Computes the [integer square root](https://en.wikipedia.org/wiki/Integer_square_root) of the [`Int{:tact}`][int] value `num`. Returns the [rounded down](https://en.wikipedia.org/wiki/Rounding#Rounding_down) result. This is equivalent to finding the largest integer `x` such that `x * x ≤ num`.
...
isqrt(1);  // 1
isqrt(0);  // 0
isqrt(-1); // ERROR! Exit code 5: Integer out of expected range
```
...
Similar to [`pow(){:tact}`](#pow), but sets the `base` to $2$. Exponent `exp` must be non-negative, otherwise an error with [exit code 5](/book/exit-codes#5) will be thrown: `Integer out of expected range`.
