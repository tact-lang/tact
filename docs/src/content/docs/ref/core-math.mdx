...
Computes the [integer square root](https://en.wikipedia.org/wiki/Integer_square_root) of the [`Int{:tact}`][int] value `num`. Returns the result rounded down (floor). This is equivalent to finding the largest integer `x` such that `x * x <= num`.
