...
Unlike [`sha256(){:tact}`](/ref/core-math#sha256), this function is gas-efficient and **only** hashes up to $127$ bytes of the given [`String{:tact}`][p]. Using longer strings would cause collisions if their first $127$ bytes are the same.

Attempts to specify a [`String{:tact}`][p] with number of bits **not** divisible by $8$ throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
