...
Strings are immutable sequences of characters, which means that once a [`String{:tact}`][p] is created, it cannot be changed. Strings are useful for storing text and can thus be converted to the [`Cell{:tact}`][cell] type to be used as message bodies.
...
Creates and returns a new [`StringBuilder{:tact}`][p] from an existing [`StringBuilder{:tact}`][p] `b`. This is useful when you need to serialize an existing [`String{:tact}`][p] to a [`Cell{:tact}`][cell] with some other data.
...
Appends the [`String{:tact}`][p] `s` to the [`StringBuilder{:tact}`][p].
...
Returns a new [`StringBuilder{:tact}`][p] after concatenating it with the [`String{:tact}`][p] `s`. Can be chained, unlike [`StringBuilder.append(){:tact}`](#stringbuilderappend).
...
Unlike [`sha256(){:tact}`](/ref/core-math#sha256), this function is gas-efficient and **only** hashes up to $127$ bytes of the given string. Using longer strings would cause collisions if their first $127$ bytes are the same.

Attempts to specify a [`String{:tact}`][p] with a number of bits **not** divisible by $8$ would throw an exception with [exit code 9](/book/exit-codes#9): `Cell underflow`.
...
Returns a [`Slice{:tact}`][slice] out of the decoded [Base64](https://en.wikipedia.org/wiki/Base64) [`String{:tact}`][p]. Alias for `self.asSlice().fromBase64(){:tact}`.
...
Returns a [`String{:tact}`][p] from a [`Slice{:tact}`][slice] by trying to load all of its bits without looking into its references, if any.
...
Note that this function is limited and only takes the first $1023$ bits of data from the given [`Slice{:tact}`][slice], without throwing an exception if the [`Slice{:tact}`][slice] has more data (i.e., when it has references).
...
More precisely, `digits` is an exponentiation parameter of the expression $10^{-\mathrm{digits}}$, which gives the represented fractional number when multiplied by the actual [`Int{:tact}`][int] value. The parameter `digits` is required to be in the closed interval: $0 < \text{digits} < 78$; otherwise, an exception with [exit code 134](/book/exit-codes#134) will be thrown: `Invalid argument`.
...
Returns a [`String{:tact}`][p] from an [`Int{:tact}`][int] value using a [fixed-point representation](https://en.wikipedia.org/wiki/Fixed-point_arithmetic) of a fractional number. It is an alias to `self.toFloatString(9){:tact}`.

This is used to represent [nanoToncoin](/book/integers#nanotoncoin) [`Int{:tact}`][int] values as strings.
