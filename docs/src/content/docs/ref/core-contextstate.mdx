...
* start with 4 zero bytes, which means the contract has received a text message that is handled in the relevant text receivers:
  * `receive("message"){:tact}`,
  * `receive(msg: String){:tact}`,
