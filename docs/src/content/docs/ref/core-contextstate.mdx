...
<Badge text="Available since Tact 1.6.14" variant="tip" size="medium"/>
<Badge text="Advanced use only" variant="danger" size="medium"/><p/>

```tact
fun setData(data: Cell);
```

This is a low-level function that directly manipulates contract storage. Only use it if you have deep understanding of:
- TON Blockchain storage architecture and TVM c4 register
- Tact's internal data serialization format and layout
- Cell construction and data encoding/decoding
- Contract upgrade patterns and migration strategies

Improper usage can permanently break your contract and make funds inaccessible.

Replaces the current contract's persistent data with the new `data` provided as parameter. This function directly modifies the contract's storage by updating the c4 register, which contains the persistent data cell.

The function is particularly useful for contract upgrades, data migrations, or when implementing custom serialization logic. However, it bypasses Tact's type safety and automatic serialization mechanisms.

**Exit codes:**

This function doesn't throw exit codes by itself, but the new data structure must be compatible with the contract's expected state format to avoid future execution failures.

:::caution

  Direct modification of contract data bypasses Tact's type safety and can lead to:
  - State corruption if data format doesn't match contract expectations
  - Loss of contract functionality if critical fields are missing or malformed  
  - Incompatibility with future contract interactions

  **Important:** After calling `setData()`, you must immediately exit the contract execution (using `return`, `throw`, or reaching the end of the receiver). If execution continues, Tact will automatically save the contract's current state fields (`self.fieldA`, `self.fieldB`, etc.) at the end, which will overwrite the data set by `setData()`.

  Always ensure the new data cell follows the exact serialization format expected by your contract's state variables.
...
contract Example {
    ...
    receive("migrate") {
        // Serialize current state with additional field
        let builder = beginCell();
        builder.storeUint(self.value, 32);
        builder.storeUint(42, 32); // new field
        
        setData(builder.endCell());
        throw(0); // REQUIRED: Must exit immediately to prevent auto-save of self.value
    }
}
```
