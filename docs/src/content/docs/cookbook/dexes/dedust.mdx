...
fun swapJetton() {
    // NOTE: Calculate wallet address of Jetton for the target user
    let targetJettonWalletAddress = address("");
    ...
    send(SendParameters{
        ...
        body: JettonTransfer{
            ...
            destination: address("EQDa4VOnTYlLvDJ0gZjNYm5PXfSmmtL6Vs6A_CZEtXCNICq_"),
...
Due to the limitations of TON, it is not possible to perform two simultaneous transfers. Therefore, the balances of user deposits must be held until LP tokens are distributed.
...
fun provideLiquidity(myJettonWalletAddress: Address) {
    let jettonMasterRaw = parseStdAddress(
        address("kQDkRHlWaibL7Tww48T6xAUFevPflca7i8TIiQTacBmnDOrb")
        .asSlice()
    );
    ...
    send(SendParameters{
        ...
        body: JettonTransfer{
            ...
            amount: jettonAmount,
            // Jetton Vault
            destination: address("kQDcUuH4xhKejjilZAIeGuBh5JRWpzbVDcO9Qfh_Q_K4q9vk"),
            responseDestination: myAddress(),
