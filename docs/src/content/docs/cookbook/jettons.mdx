...
This page lists common examples of working with [jettons](https://docs.ton.org/develop/dapps/asset-processing/jettons) and [TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md) related of knowledges.
...
Validation can be performed using the jetton wallet's state initialization and by calculating the jetton address. Note that notifications come from **your** contract's jetton wallet, so [`myAddress()`](/ref/core-common#myaddress) should be used in the owner address field. 

The wallet's initial data layout is shown below, but it may sometimes vary. Additionally, `myJettonWalletAddress` can be stored in the contract storage to reduce gas usage for each transaction.
...
To send a jetton transfer, use the [`send(){:tact}`](/book/send) function. Note that `myJettonWalletAddress` can also be stored in the contract storage to reduce gas usage for each transaction. The less on-chain computing required, the less gas you spend each time this transaction is executed.
