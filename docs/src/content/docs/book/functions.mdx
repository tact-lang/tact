---
title: Functions
description: "Global, asm, native functions, as well as receivers, getters, and member functions, plus the many attributes that allow for great flexibility and expressivity in the Tact language"
---

import { Badge } from '@astrojs/starlight/components';

Tact offers a diverse set of function kinds and attributes that provide great flexibility and expressivity. The Tact functions can be generally divided into two categories:

* [Primary](#primary), which constitute the backbone and the API of the contract
* [Complementary](#complementary), which serve as helper building blocks for reusable and maintainable contracts.
...
Primary functions form the interface of your contract, defining how it interacts with the blockchain and external systems. [Getters](#get) can be directly called from outside the blockchain, while receivers handle incoming messages from other contracts ([`receive(){:tact}`](#receive)), messages sent from off-chain ([`external(){:tact}`](#external)) or messages bounced back to the contract that sent them due to some issues on the recipients side ([`bounced(){:tact}`](#bounced)).
...
The basic receiver `receive(){:tact}` handles incoming messages from other contracts.
...
Unlike getters, method IDs for [member functions](/book/contracts#internal-functions) and some special functions are obtained sequentially: integers in the inclusive range from -4 to 0 are given to [certain message handlers](https://docs.ton.org/v3/documentation/smart-contracts/func/docs/functions#special-function-names), while member functions are numbered with method IDs starting at $1$ and going up to $2^{14} - 1$ inclusive.
...
Complementary functions serve as utilities and helper building blocks for your contracts. They cannot be directly called from outside the blockchain or handle incoming messages on their own, but they provide functionality for organizing logic of your contracts.
...
#### Member functions {#fun-member}

<Badge text="Contract scope" variant="note" size="medium"/><p/>

These functions behave similarly to private methods in popular object-oriented languages — they're internal to contracts and can be called by prefixing them with a special [identifier `self{:tact}`](/book/contracts#self). That's why member functions are sometimes referred to as "contract methods".

Member functions can access the contract's [persistent state variables](/book/contracts#variables) and [constants](/book/contracts#constants).

They can only be called from [receivers](#receive), [getters](#get), and other member functions, but not from other contracts.
...
`virtual{:tact}`  | contract | Member functions
`abstract{:tact}` | contract | Member functions
`override{:tact}` | contract | Member functions that were previously marked with `virtual{:tact}` or `abstract{:tact}`
`inline{:tact}`   | any      | All [secondary](#secondary) functions
`get{:tact}`      | contract | Member functions — turns them into [getters](#get)
...
You can allow a contract inheriting a [trait][trait] to modify a [member function](#fun-member) marked with the `virtual{:tact}` keyword by using `override{:tact}`. A function can also be marked as `abstract{:tact}`, in which case the inheriting contract must define its implementation.
...
The `abstract{:tact}` attribute allows declaring a [member function][TODO] with no body given at declaration. Functions with this attribute are meant to be overridden later in the inheritance hierarchy by either an intermediate trait or the contract inheriting it.
...
The `virtual{:tact}` attribute allows a [member function][TODO] to be overridden later in the inheritance hierarchy by either an intermediate trait or the contract inheriting it.
...
The `override{:tact}` attribute is used to override an inherited [member function][TODO] that has either a `virtual{:tact}` or `abstract{:tact}` attribute specified in the declaration of that function in one of the parent traits.
...
The `get{:tact}` attribute is a special attribute that cannot be combined with any other attribute. It is applied to [member functions], transforming them into [getter functions](#get).
...
A function call is really a call into that dictionary under that functions ID, after which:
