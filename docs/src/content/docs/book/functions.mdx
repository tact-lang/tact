...
Tact offers a diverse set of function kinds and attributes that provide great flexibility and expressivity. While some functions stand out, many of their [concepts and parts are common](#commonalities).
...
<CardGrid>
  ...
  <LinkCard
    ...
    href="#attributes-extends"
  />
  <LinkCard
    title="Extension mutation functions"
    href="#attributes-mutates"
  />
  <LinkCard
    title="Inheritance"
    href="#inheritance"
...
You can define global functions at the module, top level.
...
[Extension functions](#attributes-extends) provide a clean way to organize and extend code by allowing you to add new behaviors to existing types. The [extension _mutation_ functions](#attributes-mutates) also allow you to modify the value they operate on. Both kinds of extension functions can be called methods and both use special attributes: `extends{:tact}` and `mutates{:tact}`, respectively.
...
All kinds of functions vary in scope: some can only be defined at the top level, such as [global functions](#fun-global), while others can only be defined at the level of contracts and traits, such as [internal functions](#fun-internal) or [receivers](#receivers).

Furthermore, functions are hoisted upon declaration, such that the ones declared earlier might call those declared later and vice versa.
...
### Early return {#commonalities-return}

Execution of any function can be ended prematurely with the [`return{:tact}` statement](/book/statements#return). This is also true for the [receiver functions](#receivers), where empty `return{:tact}` statements are allowed.
...
contract YouMakeMyHeartSing() with WildThing {
    ...
    receive(_: FieldsDontLie) {
        // ...
    }
}

message FieldsDontLie {}
```
