...
Contracts and [traits][trait] have a built-in [identifier](/book/expressions#identifiers) `self{:tact}`, which is used to refer to their fields (persistent state [variables](#variables) and [constants](#variables)) and methods ([member functions](#internal-functions)):
...
* [Member functions](#internal-functions)
...
If declared or defined in a trait, member functions and constants can be marked as [virtual or abstract](/book/functions#virtual-and-abstract-functions) and overridden in contracts inheriting from the trait.
...
:::note
  ...
  To obtain the initial state of the target contract in [member functions](#internal-functions), [receivers](#receiver-functions), or [getters](#getter-functions), use the [`initOf{:tact}`](/book/expressions#initof) expression.
...
These functions behave similarly to private methods in popular object-oriented languages — they're internal to contracts and can be called by prefixing them with a special [identifier `self{:tact}`](#self). That's why member functions are sometimes referred to as "contract methods."

Member functions can access the contract's [persistent state variables](#variables) and [constants](#constants).

They can only be called from [receivers](#receiver-functions), [getters](#getter-functions), and other member functions, but not from other contracts or [`init(){:tact}`](#init-function).
...
contract Functions {
    ...
    // receiver function, which calls the member function onlyZeros
    receive("only zeros") {
        self.onlyZeros();
    }
}
...
Read more about them in their dedicated section: [Member functions](/book/functions#fun-member).
