...
In the end, this means that you cannot rely on intermediate out-of-bounds checks, because there are none, and only the last assignment of each state variable is used to update its persistent state value.
