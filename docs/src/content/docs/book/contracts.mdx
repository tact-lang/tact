...
It's difficult to determine what a contract does and which [receivers](#receiver-functions) and [getters](#getter-functions) it has without examining its source code. Sometimes the source is unavailable or inaccessible, leaving one to attempt disassembling and introspecting the contract, which is a very messy, error-prone approach with diminishing returns and no real reproducibility.
...
contract ContractParameters(
    ...
    val: Int,              // Int
    val32: Int as uint32,  // Int serialized to a 32-bit unsigned integer
