...
contract Example(
    ...
    get fun asCell(): Cell {
        return self.toCell(); // returns a Cell with the contract data
    }

    get fun asSlice(): Slice {
        return self.toSlice(); // returns a Slice with the contract data
    }
}
```
...
:::caution
    ...
    The initialization bit (in case the contract uses fields and `init()`) is not included in the resulting `Cell{:tact}` or `Slice{:tact}`.
