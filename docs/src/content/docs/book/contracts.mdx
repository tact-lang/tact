...
<Badge text="Available since Tact 1.7" variant="tip" size="medium"/> The parameters of the `init(){:tact}` function can specify [serialization formats](/book/integers#serialization) using the `as` keyword, just like you can with contract fields:
