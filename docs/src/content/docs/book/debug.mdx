...
:::note
  ...
  It's a common mistake to update `.tact` code and run tests without making a build first. That's because tests in [Blueprint][bp] rely on TypeScript wrappers generated by the Tact compiler and work with the latest build made.

  That's why every time you make a change to your Tact code, make sure to build it with `npx blueprint build` before you execute the test suite. For your convenience, you may combine builds and tests into a single command, as shown in the [experimental lab setup](#lab-4).
...
To send messages to contracts, use the `.send(){:typescript}` method on their TypeScript wrappers like so:
