...
In addition to dumping values, it's often helpful to use assertive functions like [`require(){:tact}`](/ref/core-debug#require), [`throwIf(){:tact}`](/ref/core-debug#throwif) and [`throwUnless(){:tact}`](/ref/core-debug#throwunless). They help stating your assumptions clear, and are handy for setting up "trip wires" for catching issues in the future.
