...
If you want to pin down a specific version of the compiler, simply specify it in your [project.json](https://docs.npmjs.com/cli/v10/configuring-npm/package-json) as follows: `"@tact-lang/compiler": "X.Y.Z"{:json}`, where `X.Y.Z` is the version that you want to set.
...
:::caution
  ...
  For example, using the `npm` package manager to install dependencies and then using `yarn` to build the contracts can lead to the following error message:
