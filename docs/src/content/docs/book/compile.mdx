...
By default, Tact automatically generates test stubs for each compiled contract. These files provide a basic structure for testing your contracts using TypeScript and the TON blockchain sandbox.

#### Generated file structure {#test-file-structure}

Test stubs are saved to the `tests/` subdirectory inside the output folder specified in the [configuration](/book/config#projects-output). For each contract, a file is created with the name following the template:
...
```
build/tests/MyProject_Counter.stub.tests.ts
```
...
- Test stubs for contract deployment
- Examples using the [TypeScript wrappers](#wrap-ts)

Example of a generated test stub:
...
You can disable automatic test generation by adding the `skipTestGeneration` option to your [project configuration](/book/config):
...
#### Using test stubs {#using-test-stubs}
...
:::note
  ...
  See how to use the generated tests for [debugging and testing your contracts](/book/debug#tests).
