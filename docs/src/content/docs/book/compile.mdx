...
Test stubs are saved to the `tests/` subdirectory inside the output folder specified in the [configuration](/book/config#projects-output). Each contract gets its own test file named:

```
{project_name}_{contract_name}.stub.tests.ts
```

For example: `build/tests/MyProject_Counter.stub.tests.ts`
