...
There are no implicit type conversions in Tact, so addition ([`+{:tact}`](/book/operators#binary-add)) of two boolean values isn't possible. However, many comparison [operators](/book/operators) are available, such as:
