...
:::danger
  ...
  Current assembly functions are highly experimental and may change in future releases. Please be sure to follow our updates in the [Tact Kitchen](https://t.me/tact_kitchen) or monitor the release changelogs to see when we will update this feature.
...
### Don't rely on order of fields in structures in the standard library {#caveats-stdlib-structures}

To reduce the number of stack-manipulating statements and to save gas, the order of the fields of structures in the standard library may change between releases. Such changes do not affect any other kinds of functions, expect for `asm{:tact}` functions.

That's because assembly functions, unlike all others, require a certain order of values on the stack before and after their execution. If this order is disrupted, the whole logic of such function can be broken in an instant.

Hence, when writing assembly functions, make sure that you are not using the built-in [Structs] and [Messages] from Tact's library. If you want to target a specific structure, make a copy of it in your code, and then use that copy so that it won't be affected by future Tact updates.
