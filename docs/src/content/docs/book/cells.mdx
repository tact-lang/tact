...
There, `^cell`, `^builder` and `^slice` in [TL-B][tlb] syntax mean the reference to [`Cell{:tact}`](#cells), [`Builder{:tact}`](#builders) and [`Slice{:tact}`](#slices) values respectively, while the `cell`, `builder` or `slice` tells that the given value would be stored as a `Cell{:tact}`, `Builder{:tact}` or `Slice{:tact}` directly and not as a reference.
