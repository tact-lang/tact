...
struct TwoCell {
    ...
    cRem: Cell as remaining; // remainder<cell> in TL-B,
                             // see explanation for those below
}
...
There, `^cell`, `^builder` and `^slice` in [TL-B][tlb] syntax mean the reference to [`Cell{:tact}`](#cells), [`Builder{:tact}`](#builders) and [`Slice{:tact}`](#slices) values respectively, while the `remainder<…>` of `cell`, `builder` or `slice` tells that the given value would be stored as a `Cell{:tact}`, `Builder{:tact}` or `Slice{:tact}` directly and not as a reference. Note that the `remainder<X>` is not part of the regular [TL-B][tlb] syntax and is only present in Tact-generated comments in various [build artifacts](/book/compile#artifacts) as a visual clue.
