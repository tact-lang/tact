...
While you may use them for [manual construction](#cnp-manually) of cells, it's strongly recommended to use [Structs][struct] instead: [Construction of cells with Structs](#cnp-structs).
...
While you may use slices for [manual parsing](#cnp-manually) of cells, it is strongly recommended to use [Structs][struct] instead: [Parsing cells with Structs](#cnp-structs).
...
```tact
struct Example {
    b32: Slice as bytes32; // 256 bits with arbitrary contents
    u256: Int as uint256;  // also 256 bits, but viewed as a single integer
}
```
...
```tact
struct Example {
    b64: Slice as bytes64; // 512 bits with arbitrary contents
}
```

For an advanced usage example, see the [`SignedBundle`](/ref/core-crypto#signedbundle) [struct][struct].
...
It is strongly recommended to use them and their [methods](/book/functions#extension-function), such as [`Struct.toCell(){:tact}`][st-tc] and [`Struct.fromCell(){:tact}`][st-fc], instead of manually constructing and parsing cells, as this allows for much more declarative and self-explanatory contracts.

The examples of manual parsing [above](#cnp-manually) can be rewritten using [Structs][struct], with descriptive names for fields if desired:
...
By using [Structs][struct] and [Messages][message] instead of manual [`Cell{:tact}`](#cells) composition and parsing, those details will be simplified away and will not cause any hassle when the optimized layout changes.
