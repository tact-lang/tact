...
If set to `true{:json}`, enables run-time null checks of the arguments of the unwrapping `!!` operator. Setting the option to `false{:json}` will disable these checks and decrease gas consumption.
