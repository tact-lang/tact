...
`"disable"{:json}` by default.

If not set to `"disable"{:json}`, stores internal and external receivers outside the methods map.

When enabled, it saves gas but can cause the contract to be incorrectly recognized and misparsed by some explorers and user wallets. To ensure compatibility at the cost of fewer gas optimizations, use the `"explorers-compatible"{:json}` option.
