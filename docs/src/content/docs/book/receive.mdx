...
Reentrancy of messages is not possible — despite message passing being asynchronous, processing a single message by a single receiver is always synchronous except for [outbound message processing](/book/send#outbound-message-processing). That said, all pending messages on TON Blockchain are publicly visible, and an attacker can spam your contract with identically composed messages. Read more about how to prevent it here: [Preventing front-running with signature verification](/book/security-best-practices/#preventing-front-running-with-signature-verification).
