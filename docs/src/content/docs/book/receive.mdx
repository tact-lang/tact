...
TON is a distributed blockchain, which means that communication between contracts is performed by sending and receiving messages. The most common type of message is the _internal message_ 0 a message sent from one contract (or a wallet) to another.
