...
A single received message equates to a single transaction. That is, no other message can influence the flow of the current transaction initiated by the incoming message. On top of that, due to the [split into different phases](/book/send#outbound-message-processing), outgoing messages are not sent until the end of computations within the current transaction. Thus, reentrancy of messages is not possible.

That said, all pending messages on TON Blockchain are publicly visible, and an attacker can spam your contract with identically composed messages. Read more about how to prevent it here: [Preventing front-running with signature verification](/book/security-best-practices/#preventing-front-running-with-signature-verification).
