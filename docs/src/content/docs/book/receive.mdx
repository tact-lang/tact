...
Furthermore, to make an early exit from the receiver without saving the new contract's state, use the [`throw(0){:tact}`](/ref/code-debug#throw) idiom. It will immediately and successfully terminate the execution of the compute phase of the contract and skip the code generated by Tact to save the contract's state. Conversely, changes to data made before calling `throw(){:tact}` in with this function will be lost unless you manually save them beforehand.
