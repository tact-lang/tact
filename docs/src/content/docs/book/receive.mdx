...
A single received message equates to a single transaction. That is, no other message can influence the flow of the current transaction initiated by the incoming message. On top of that, [message-sending functions](/book/send#message-sending-functions) and underlying TVM instructions do not deliver messages immediately but instead [queue them to be sent after the end of computations](/book/send#outbound-message-processing) in a separate phase of the transaction. Thus, [reentrancy](https://en.wikipedia.org/wiki/Reentrancy_(computing)) of messages is not possible.
