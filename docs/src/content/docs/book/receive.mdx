...
Re-entrancy of messages is possible — when processing a message, a contract might get another identically composed one. Read more about it here: [Preventing front-running with signature verification](https://docs.tact-lang.org/book/security-best-practices/#preventing-front-running-with-signature-verification).
