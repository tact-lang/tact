...
The `let{:tact}` statement allows local and [block](#block)-scoped variable declaration. In Tact, variable declarations are also their definitions and **require** an initial value.
