...
```tact
extends mutates fun correct(self: Int) {
    if (self == 42) {
        return;
    }
...
This is done to help avoid potential logical errors in the code.

```tact
fun doubleTrouble(): Int {
    return 10;
    return 20; // COMPILATION ERROR! Unreachable statement
}
```
