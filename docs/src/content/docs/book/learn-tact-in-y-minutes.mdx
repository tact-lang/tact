...
trait MyTrait with Ownable {
    ...
    // are often called member functions.
    //
    ...
    // Adding an abstract attribute to the member function requires us
    // to omit their body definitions and demand that from contracts that
    // will inherit the trait.
    abstract fun trust(msg: MyMsg);

    // Adding a virtual attribute to the member function allows their
...
contract MyContract(
    ...
    // functions and it MUST override its abstract member functions as to provide
