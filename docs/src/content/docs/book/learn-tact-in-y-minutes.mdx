...
fun showcase() {
    ...
    // Type ascription is mandatory unless you use a map literal for initialization.
    ...
    // Since Tact 1.6.7 statically known `set`s can be replaced by map literals.
    // Because the map is created at compile-time, it doesn't consume as much gas.
