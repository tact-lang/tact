...
fun showcase() {
    ...
    // This is very expensive on-chain, do such calculations off-chain:
...
fun showcase() {
    ...
    let anotherMap: map<Int, Int> = emptyMap();
    myMap.deepEquals(anotherMap); // true, and for the explanation of it see the
...
struct AnyStruct { _: Int }
message AnyMessage { _: Int }
```
...
trait MyTrait {
    const iAmOnTheTraitLevel: Int = 420;
    ...
    virtual const youCanOverrideMe: Int = crc32("babecafe");
}
...
contract MyContract() with MyTrait {
    ...
    // the iAmOnTheTraitLevel constant is also in scope of this contract,
    // but we cannot override it.

    // However, we can override the virtual constant.
    override const youCanOverrideMe: Int = crc32("deadbeef");

    // And we MUST override and define the value of the abstract constant.
    override const overrideMe: Int = ton("0.5");
}
