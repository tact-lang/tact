...
---------: | :----------------------------------- | -----------
$+1$       | ~~`SendPayGasSeparately{:tact}`~~    | <Badge text="Deprecated since Tact 1.6.5" variant="tip" size="medium"/><p/> Pay [forward fees][fwdfee] separately from the message value.
