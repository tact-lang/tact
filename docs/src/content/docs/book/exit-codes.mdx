...
This error can be triggered by [manual construction](/book/cells#cnp-manually) of the cells via [relevant `Builder.storeSomething(){:tact}` methods](/ref/core-cells#builder) or when [using structs and Messages and their convenience methods](/book/cells#cnp-structs).
