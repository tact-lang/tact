...
Map literals are expressions that provide a concise way to create maps. First goes the `map<K, V>{:tact}` type expression, then a curly-brace enclosed comma-delimited list of zero or more predefined key-value pairs.

```tact
// Declaring a map as a local variable via a map literal expression
let myMap: map<Int as uint8, Int as uint8> = map<Int as uint8, Int as uint8> {
    // Key: Value
    1: 100, // key 1, value 100
    2: 200, // key 2, value 200
};

// Notice that type ascription can be omitted since the type can be inferred
let inferredMap = map<Int as uint8, Int as uint8> {
    1: 100,
    2: 200, // trailing comma is allowed
};
...
```tact
let myMap = map<Int, Cell> {};
let myMap2: map<Int, Cell> = emptyMap();
...
Because maps are represented as cells with binary data, `Int` and `Uint` types must be used with [storage annotations](/book/integers#serialization).

```tact
let smartMoney = map<Int as uint8, Int as coins> {};
...
You can check if a key is found in the map by calling the `.get(){:tact}` [method](/book/functions#extension-function), which is accessible for all maps. This will return [`null{:tact}`](/book/optionals) if the key is missing or the value if the key is found.
...
To replace the value associated with a key, if such a key exists, use the `.replace(){:tact}` [method](/book/functions#extension-function). It returns `true{:tact}` upon successful replacement and `false{:tact}` otherwise.
...
To delete a single key-value pair (a single entry), use the `.del(){:tact}` [method](/book/functions#extension-function). It returns `true{:tact}` in the case of successful deletion and `false{:tact}` otherwise.
...
The `.exists(){:tact}` [method](/book/functions#extension-function) on maps returns `true{:tact}` if a value under the given key exists in the map and `false{:tact}` otherwise.
