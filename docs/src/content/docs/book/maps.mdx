...
* [`Cell{:tact}`][cell]
* [`Address{:tact}`][p]
* any [struct][struct] type (Note: while structs can be values in maps, using them directly in [map literals](#initialize) is currently unsupported)
...
:::note
While `struct` types are generally allowed as map values (see [Allowed value types](#allowed-types)), they are currently **not supported** directly within map literals. Attempting to use a `struct` as a value in a map literal will result in a compiler error.
:::
