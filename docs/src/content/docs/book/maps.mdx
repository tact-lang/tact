...
Map literals (or initializers) are expressions that provide a concise way to create maps. First goes the `map<K, V>{:tact}` type expression, then a curly-brace enclosed comma-delimited list of zero or more predefined key-value pairs.

```tact
// Declaring a map as a local variable via a map literal expression
let myMap: map<Int, Int> = map<Int, Int> {
    // Key: Value
...
```tact
let putItBluntly = map<Int, Int> {
    1: 100,
    1: 200,
    1: 300,
    1: 420,
};
putItBluntly.get(1)!!; // 420
```
...
Finally, serialization annotations of key and value types are allowed with the [`as{:tact}` keyword](/book/integers#serialization).
