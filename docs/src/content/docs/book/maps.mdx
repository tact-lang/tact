...
```tact
// K and V correspond to the key and value types of the given map
extends mutates fun add(self: map<K, V>, key: K, val: V): Bool;
```

To add a value to a map, call the `.add(){:tact}` [method](/book/functions#extension-function). If the key already exists, do nothing and return `false{:tact}`. Otherwise, add the value and return `true{:tact}`.
...
// Adding a value
let added1 = fizz.add(7, 7); // true
fizz.add(7, 7);              // false, because the key 7 already exists
```
