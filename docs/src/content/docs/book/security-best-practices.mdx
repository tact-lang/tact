...
Don't use randomization in `external_message` receivers, as it remains vulnerable even with randomized logical time.
...
Alternatively, you can leverage the standard [`notify(){:tact}`](/ref/core-base/#self-notify) or [`forward(){:tact}`](/ref/core-base/#self-forward) functions.
