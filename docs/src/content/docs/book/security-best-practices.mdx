...
Front-running can occur when an attacker observes a pending transaction containing a valid signature and quickly submits their own transaction using the same signature before the original transaction is processed. This is particularly relevant in scenarios where signatures authorize actions or fund transfers.
...
message(0x988d4037) Request {
    ...
    to: Address; // Recipient address is part of the message
    seqno: Int as uint64;
    // Other data fields...
}
...
contract SecureChecker(
    ...
    receive(request: Request) {
        // Construct the data that was originally signed, including 'to' address
        let dataToVerify: Cell = beginCell()
            .storeAddress(request.to)
            // Store other data fields...
            // Store seqno as well for replay protection
            .storeUint(self.seqno, 64)
            .endCell();

        // Verify the signature against the reconstructed data hash
        require(checkSignature(dataToVerify.hash(), request.signature, self.public_key), "Invalid signature!");

        // Check replay protection
        require(request.seqno == self.seqno, "Invalid seqno"); // Assuming external message with seqno
        ...
        message(MessageParameters{
            to: request.to, // Use the 'to' from the signed request
            value: 0,
    ...
    // Getter for seqno might be needed depending on the replay protection mechanism
    get fun seqno(): Int {
        return self.seqno;
    }
}
