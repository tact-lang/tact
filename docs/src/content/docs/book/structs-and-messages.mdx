...
Creation of [Struct](#structs) and [Message](#messages) instances resembles [function calls](/book/expressions#static-function-call), but instead of parentheses `(){:tact}` one needs to specify arguments in braces `{}{:tact}` (curly brackets):
