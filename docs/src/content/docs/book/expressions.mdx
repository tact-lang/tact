...
If you only need the code of the given contract and not its [`StateInit{:tact}`](#initof), prefer using `codeOf ContractName{:tact}` over [`initOf ContactName(param1, param2, ...){:tact}`](#initof) to **significantly** reduce gas usage.
