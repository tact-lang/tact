...
You can also call [extension functions](/book/functions#extension-function) defined on certain [primitive types][p] directly on their corresponding literal values:
...
Map literals also known as map initializers, concisely create maps by enclosing a comma-delimited list of zero or more predefined key-value pairs in curly braces.

```tact
// A compile-time map literal
let myMap = map<Int as uint8, Int as int13> {
