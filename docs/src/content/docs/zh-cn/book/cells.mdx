...
虽然 [TVM][tvm] 类型 [`Cell{:tact}`](#cells) 指的是所有cell，但存在不同的cell类型，其内存布局也各不相同。 [前面](#cells) 描述的通常被称为 _普通_ (或简单) cell--这是最简单、最常用的cell，只能包含数据。 绝大多数关于cell及其用法的描述、指南和 [参考文献](/ref/core-cells) 都假定是普通cell。
...
- [Library reference cell][c-library]，子类型编码为 $2$ - 它们用于存储库，通常在 [masterchain](/book/masterchain) 上下文中使用。
...
- 作为[合约](/book/contracts)和[特性](/book/types#traits)的[存储变量](/book/contracts#variables)，
...
:::note
  ...
  注意，通过 `as remaining{:tact}` 序列化的cell不能是 [可选](/book/optionals)。  也就是说，指定类似 `Cell? as remaining{:tact}`, `Builder? as remaining{:tact}` 或 `Slice? as remaining{:tact}` 会导致编译错误。
...
强烈建议使用它们及其 [方法](/book/functions#extension-function)，如 [`Struct.toCell(){:tact}`][st-tc]和 [`Struct.fromCell(){:tact}`][st-fc]，而不是手动构造和解析cell，因为这样可以得到更多声明性和不言自明的合约。
...
[st-fcc]: /ref/core-cells#structfromcell
...
:::note[Useful links:]
  ...
  [核心库中的 `Cell.asSlice(){:tact}`](/ref/core-cells#cellasslice)\
  [核心库中的 `Builder.asSlice(){:tact}`](/ref/core-cells#builderasslice)\
...
不能使用二进制相等 [`=={:tact}`][bin-eq] 或不等式 [`!={:tact}`][bin-eq] 操作符直接比较 [`Builder{:tact}`](#builders) 类型的值。 但是，[`cell{:tact}`](#cell)和[`slice{:tact}`](#slice)类型的值可以。
