...
其他类型的细胞统称为_外来细胞（或特殊细胞）。 它们有时会出现在 TON 区块链上的区块和其他数据结构的实际表示中。 它们的内存布局和用途与普通电池大不相同。
...
:::note[Useful 链接：]
  ...
  [TON Docs 中的简单证明验证示例][mproof] [TON Docs 中的简单证明验证示例][mproof]\

:::

[c- 修剪]: https://docs.ton.org/develop/data-formats/exotic-cells#pruned-branch
...
[防潮]: https://docs.ton.org/develop/data-formats/exotic-cells#simple-proof-verifying-example
...
- _Refs descriptor_ 的计算公式如下： $r + 8 _ k + 32 _ l$，其中 $r$ 是单元格中包含的引用数（介于 $0$ 和 $4$之间）， $k$ 是单元格类型标志（$0$ 表示 [ordinary](#cells-kinds) 和 $1$ 表示 [exotic](#cells-kinds) ）， $l$ 是单元格的 [level](#cells-levels) （介于 $0$ 和 $3$之间）。
...
此外，还有专门针对 [exotic](#cells-kinds) 单元格的指令来创建这些单元格并期望它们的值。 不过，[普通](#cells-kinds)单元格解析指令仍可用于[奇异](#cells-kinds)单元格，在这种情况下，它们会在反序列化尝试中被自动替换为[普通](#cells-kinds)单元格。

所有单元格操作指令都需要将 [`单元格{:tact}`](#cells)类型的值转换为 [`构建器{:tact}`](#builders)或 [`切片{:tact}`](#slices)类型，然后才能修改或检查这些单元格。
...
生成器{:tact}\` 类型表示部分组成的单元格，为其定义了追加整数、其他单元格、引用其他单元格等快速操作：
...
其中，[TL-B][tlb] 语法中的 `^cell`、`^builder` 和 `^slice` 分别表示对 [`cell{:tact}`](#cells)、[`builder{:tact}`](#builder)和 [`slice{:tact}`](#slices)值的引用、而 `cell`、`builder` 或 `slice` 的 `remainder<…>` 则表示给定值将直接存储为 `Slice{:tact}`，而不是作为引用。
...
收到 [jetton][jetton] 传输通知消息后，其单元体会被转换为 [`片{:tact}`](#片)，然后解析为 `JettonTransferNotification{:tact}` [消息][消息]。 在此过程结束时，"forwardPayload "将包含原始信息单元的所有剩余数据。

在这里，将 `forwardPayload：Slice as remaining "字段放在 `JettonTransferNotification{:tact}\` [信息][message]中的任何其他位置都不会违反[jetton][jetton]标准。 这是因为 Tact 禁止在[Structs][结构]和[Messages][消息]的最后一个字段之外的任何字段中使用 "as remaining{:tact}"，以防止滥用合同存储空间并减少气体消耗。
...
请注意，Tact 的自动布局算法是贪婪的。 例如，`struct Adventure{:tact}` 占用的空间很小，它不会以引用 [`单元格{:tact}`](#单元格) 的形式存储，而是直接以 [`切片{:tact}`](#切片) 的形式提供。

通过使用 [结构][structs] 和 [消息][messages]，而不是手动 [`Cell{:tact}`](#cells)组成和解析，这些细节将被简化，在优化布局发生变化时也不会造成任何麻烦。
...
:::note[Useful 链接：]
  ...
  [`Struct.fromSlice() ` in Core library][st-fs]\ [`Message.toCell() ` in Core library][msg-tc]\ fromSlice(){:tact}` 在核心库中][st-fs]\
...
细胞{:tact}`](#cells)和 [建筑{:tact}`](#builders)都不能直接检查空性，需要先将它们转换为 [切片{:tact}\`](#slices)。

要检查是否有任何位，请使用 [`Slice.dataEmpty(){:tact}`][s-de]。 要检查是否存在引用，请使用 [`Slice.refsEmpty(){:tact}`][s-re]。 要同时检查这两个文件，请使用 [`Slice.empty(){:tact}`][s-e]。
...
:::note[Useful 链接：]
  ...
  [`Cell.asSlice(){:tact}` 核心库中](/ref/core-cells#cellasslice)/
  [`Builder.asSlice(){:tact}` 核心库中](/ref/core-cells#builderasslice)/
  [`Slice.dataEmpty() ` 核心库中][s-de]/ [`Slice.refsEmpty() ` 核心库中][s-re]/ dataEmpty(){:tact}` 在核心库中][s-de]\
...
### 检查是否等于 {#operations-equal}

不能使用二进制相等 [`=={:tact}`][bin-eq]或不等式 [`！={:tact}`][bin-eq]操作符直接比较 [`生成器{:tact}`](#builders)类型的值。 但是，[`单元{:tact}`](#单元)和[`切片{:tact}`](#切片)类型的值可以。
...
[接收]: /book/contracts#receiver-functions
[tvm]: https://docs.ton.org/learn/tvm-instructions/tvm-overview
[tlb]: https://docs.ton.org/develop/data-formats/tl-b-language
[杰顿]: https://docs.ton.org/develop/dapps/asset-processing/jettons
