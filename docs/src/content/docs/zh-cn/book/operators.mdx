...
:::note
  ...
  二进制相等 `=={:tact}` 和不等式 `!={:tact}` 运算符通过[`.hash(){:tact}`](/zh-cn/ref/core-cells#cellhash)函数，隐式地通过各自[cell][cell]的哈希值比较[maps](/zh-cn/book/maps)。 虽然这在大多数情况下没有问题，因为大多数映射序列化器的工作原理与 TON 区块链源中的序列化器相同，但通过手动序列化映射或更改某些库中序列化器的逻辑，仍有可能得到错误的负面结果。
