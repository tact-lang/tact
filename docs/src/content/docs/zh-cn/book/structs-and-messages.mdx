...
:::caution
  ...
  **警告**：目前无法**\*循环类型。 这意味着结构/消息 **A** 的字段不能与结构/消息 **B\*\* 的字段相同。
...
:::note
  ...
  更深入的信息请参见：
  [Convert received messages to `op` operations](/book/func#convert-received-messages-to-op-op-operations)
...
无需通过一系列相关的 `.loadSomething(){:tact}` 函数调用来手动解析 [`单元格{:tact}`][单元格] 或 [`切片{:tact}`][切片]，而是可以使用 `.fromCell(){:tact}` 和 `.fromSlice() ` [扩展函数](/book/functions#extension-function)。fromSlice(){:tact}`[扩展函数](/book/functions#extension-function)将所提供的 [`Cell{:tact}`][单元格] 或 [`Slice{:tact}\`][切片] 转换为所需的 [Struct](#structs) 或 [Message](#messages) 。

这些扩展函数仅尝试根据 [Struct](#structs) 或 [Message](#messages) 的结构解析 [`单元格{:tact}`][单元格] 或 [`片{:tact}`][片]。 如果布局不匹配，可能会抛出各种异常--确保用 [`try...catch{:tact}`](/book/statements#try-catch)块封装代码，以防止意外结果。
