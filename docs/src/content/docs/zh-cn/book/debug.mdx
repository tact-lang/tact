...
除了转储值之外，使用一些断言函数通常也很有帮助，例如 [`require(){:tact}`](/ref/core-debug#require)、[`nativeThrowIf(){:tact}`](/ref/core-debug#nativethrowif) 和 [`nativeThrowUnless(){:tact}`](/ref/core-debug#nativethrowunless)。 它们有助于明确说明你的假设，并方便设置 "绊线"，以便在将来发现问题。
...
为了使 [`dump(){:tact}`][dump]或 [`dumpStack(){:tact}`](/ref/core-debug#dumpstack)等函数正常工作，需要启用调试模式。

最简单和推荐的方法是修改项目根目录下的 [`tact.config.json`](/book/config) 文件（如果还不存在，则创建该文件），并 [将 `debug` 属性设置为 `true{:json}`](/book/config#options-debug)。
...
除此之外，[`tact.config.json`](/book/config) 仍然可以用于 [Blueprint][bp] 项目。 除此之外，[Blueprint][bp] 项目中仍可使用 [`tact.config.json`](/book/config)。 在这种情况下，除非在 `wrappers/` 中修改，否则 [`tact.config.json`](/book/config)中指定的值将作为默认值。
...
:::note
  ...
  阅读更多关于配置和 [`tact.config.json`](/book/config) 文件的信息：[配置](/book/config)。\
...
方法 `expect(…).toEqualCell(){:typescript}` 检查两个 [cell](/book/cells#cells)是否相等：
...
[沙盒][sb]提供了一个辅助函数 `printTransactionFees(){:typescript}`，它可以漂亮地打印所提供交易的所有值和费用。  它对观察 [nano Toncoins](/book/integers#nanotoncoin）的流动非常方便。
...
有时，进行负面测试也很有用，它可以故意出错并抛出特定的[退出码](/book/exit-codes)。
...
要查看字典中 [`emit(){:tact}`](/ref/core-common#emit)的日志，请查找没有收件人的外部消息。  在各种 TON 区块链探索器中，此类交易将被标记为 "外部输出(external-out)"，目的地指定为"-"或 "空"。
...
当 [sent](/book/send) 带有 `bounce: true{:tact}`时，消息可以在出现错误时退回. 确保编写相关的 [`bounced(){:tact}`](/book/bounced)消息[接收器](/book/contracts#receiver-functions)，并优雅地处理被退回的消息：
...
了解更多关于退回消息和接收者的信息：[退回消息](/book/bounced)。
...
4. #### 我们来测试一下！ 我们来测试一下！ {#lab-4}
   ...
   从现在起，您只需修改 Tact 合约文件中已测试的 [receiver function](/book/contracts#receiver-functions) 的内容，然后重新运行上述命令，就可以对某些内容进行测试。  重复该过程，直到您测试了想要测试的内容。
