...
Declaring variable always requires an initial value and an explicit type: However, the type ascription can be omitted and Tact will try to infer it from the initial value:
