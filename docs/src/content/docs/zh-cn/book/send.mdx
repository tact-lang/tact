...
| `value`  | [`Int{:tact}`][int]    | 消息中要发送的[nanoToncoins][nano]的金额。 此值通常用于支付[转发费用][fwdfee]，除非使用了可选标志[`SendPayGasSeparately{:tact}`](/book/message-mode#optional-flags)。 |
| `mode`   | [`Int{:tact}`][int]    | 一个 8 位值，用于配置发送消息的方式，默认值为 $0$。 参见：[消息`模式`](/book/message-mode)。 见：[消息`mode`](/book/message-mode).                                    |
| `body`   | [`Cell?{:tact}`][cell] | [可选][opt]消息正文作为[`Cell{:tact}`][cell]                                                                                                |
| `code`   | [`Cell?{:tact}`][cell] | [可选][opt] 合约的初始代码（编译后的字节码）                                                                                                          |
| `data`   | [`Cell?{:tact}`][cell] | [可选][opt]合约的初始数据（合约的[`init(){:tact}`函数](/book/contracts#init-function)的参数）                                                          |

字段 `code` 和 `data` 被称为[初始化包](/book/expressions#initof)，它用于新合约的部署。
...
[可选标记](/book/message-mode#optional-flags) `SendIgnoreErrors{:tact}`表示即使在发送消息过程中发生错误，也会继续发送下一条消息。 **在发送阶段，没有错误会导致交易回滚。**
...
要部署一个合约，你需要用 [`initOf{:tact}`](/book/expressions#initof)计算它的地址和初始状态，然后在初始化消息中发送它们：
...
TON 区块链上的每笔交易都由 [多个阶段][phases] 组成。 发送消息是在[计算阶段][compute]进行评估，但是在该阶段**不是**发送。 相反，它们将按照出现顺序排入[操作阶段][phases]，在[计算阶段][compute]中列出的所有操作，如外向消息或[储备请求](/ref/core-advanced#nativereserve)，都会在此阶段执行。
...
- 在出站消息处理过程中，假设入站消息中提供了足够的金额，第一条消息会在余额上留下 $\mathrm{R} - (0.042 + \mathrm{forward\_fees})$ [nanoToncoins](/book/integers#nanotoncoin) 。

- 处理第二条消息时，合约会尝试发送 $\mathrm{R}$ [nano Toncoins](/book/integers#nanotoncoin)，但发送失败，因为剩余的金额已经较少。
