...
使用[十六进制数字系统](https://en.wikipedia.org/wiki/Hexadecimal)表示数字，用$\mathrm{0x}$（或$\mathrm{0X}$）前缀表示：$\mathrm{0xFFFFFFFFF}.$\04 使用下划线（`_`）提高可读性：$\mathrm{0xFFF\_FFFF\_FFF}$等于$\mathrm{0xFFFFFFFFF}.$

### 八进制

使用[八进制数字系统](https://en.wikipedia.org/wiki/Octal)表示数字，用$\mathrm{0o}$（或$\mathrm{0O}$）前缀表示：$\mathrm{0o777777777.}$\04 使用下划线（`_`）提高可读性：$\mathrm{0o777_777\_777}$等于$\mathrm{0o777777777}.$

### 二进制

使用[二进制数字系统](https://en.wikipedia.org/wiki/Binary_number)表示数字，用 $\mathrm{0b}$（或 $\mathrm{0B}$）前缀表示： $\mathrm{0b111111111.}$\04 使用下划线（`_`）提高可读性： $\mathrm{0b\_111\_111}$ 等于 $\mathrm{0b111111111}.$
...
同样，在使用 TON 区块链的主要货币 Toncoin 时，需要九位小数，而不是两位小数。 可以说，纳米通币是通币的 $\frac{1}{10^{9}}\mathrm{th}$。

因此， $1.25$ Toncoin 的数量，可以用 Tact 表示为 [`ton("1.25"){:tact}`](/ref/core-comptime#ton)，实际上就是数字 $1250000000$。 我们把这些数字称为 _ 纳米吨币_（或 _ 纳米吨_），而不是 _ 美分_。
...
| `coins{:tact}`   | 16`][varUInteger][varuint][`VarUInteger 16 | $0$ 到 $2^{120} - 1$        | $4$ 和 $124$ 位之间，[见下文]（#序列化-币值） |
...
这里，"oneByte "被序列化为[`uint8`](#serialization-types)，只占用一个字节，范围从 $0$ 到 2^{8} - 1$，即 $255$。 在运行时计算中使用时，不会发生溢出，所有计算结果都是 $257$- 位有符号整数。 但是，就在我们决定将 `tmp` 的值存储回 `oneByte` 的那一刻，我们收到了一个错误，[退出代码 5](/book/exit-codes#5)，错误信息如下：整数超出预期范围。

:::caution
  因此，在使用序列化时，对数字要\*\*\*\*小心，并始终仔细检查计算结果。
:::
