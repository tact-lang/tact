...
将 [`片{:tact}`][片] 的内部 DNS 表示规范化。 传递的 [`Slice{:tact}`][slice]必须没有任何引用，否则将产生[退出代码 134](/book/exit-codes#134)异常："无效参数"。
...
合同 ExampleContract with DNSResolver {
    // 现在，这个合同有一个：
