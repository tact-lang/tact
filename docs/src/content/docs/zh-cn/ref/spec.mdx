...
```
Shiki 在通过 TextMate 语法高亮 Ohm 方面做得很差，尽管在 VSCode 中运行良好。我认为原因可能在于 Shiki 是逐行高亮的，或者它使用了不同的 RegEx 引擎。我不确定问题出在哪里，所以在问题解决之前，最好使用 tree-sitter-ohm 并使用其 `highlight` 命令生成 ANSI 序列。剩下的就交给 Shiki 吧：
```
