...
:::note
  ...
  每个用户都建议使用 [结构体][struct] 及其 [方法](/book/functions#extension-function)，例如 [`Struct.toCell(){:tact}`](#structtocell) 和 [`Struct.fromCell(){:tact}`](#structfromcell)，而不是手动构建和解析单元格，因为 [结构体][struct] 和 [消息][message] 最接近于作为你合约的 [活的 TL-B 模式](/book/cells#cnp-structs)。
...
尝试存储一个负数 `value` 或提供一个不足或超出范围的 `bits` 数量会抛出异常，错误代码为 [退出代码 5](/book/exit-codes#5): `Integer out of expected range`。
...
试图提供一个不足或超出范围的`比特`数时，会出现[退出码5](/book/exit-codes#5)的异常：`Integer out of expected range`。
...
试图存储一个超出范围的`值`时，会出现[退出码5](/book/exit-codes#5)的异常：`Integer out of expected range`。

这是保存 [nanoToncoins](/book/integers#nanotoncoin) 的最常见方式。
...
:::note[Useful links:]
  ...
  [特殊的 `coins` 序列化类型](/book/integers#serialization-coins)
...
由于单个 [`cell{:tact}`][cell] 最多可存储 $4$ 引用，如果尝试存储更多引用，则会出现[退出码 8](/book/exit-codes#8)异常：`Cell overflow`。
...
由于单个 [`Cell{:tact}`][cell] 最多可以存储 $4$ 个引用，尝试存储更多引用将抛出异常，错误码为 [exit code 8](/book/exit-codes#8)：`Cell overflow`。
...
试图指定一个超出范围的 `l` 值时，会出现 [exit code 5](/book/exit-codes#5)异常：`Integer out of expected range`。

尝试加载的数据超过 [`Slice{:tact}`][slice] 包含的数据时，会出现 [exit code 9](/book/exit-codes#9) 异常：`Cell underflow`。
...
尝试预载的数据超过 [`Slice{:tact}`][slice] 所包含的数据时，会出现 [exit code 9](/book/exit-codes#9)异常：`Cell underflow`。
...
尝试加载的数据超过 [`Slice{:tact}`][slice] 所包含的数据时，会出现 [exit code 9](/book/exit-codes#9) 异常：`Cell underflow`。
...
当 [`Bool{:tact}`][bool][`slice{:tact}`][slice]不包含它时，尝试加载此类 [`Bool `][bool]会产生异常，[退出码 8](/book/exit-codes#8)：`Cell overflow`。
...
加载并返回 [序列化](#builderstorecoins)一个范围为 $0 ... 的无符号 [`Int{:tact}`][int]值。 2^{120} - 1$ 来自 [`Slice{:tact}`][slice]。 此值通常代表 [nanoToncoins](/book/integers#nanotoncoin) 的金额。

当 [\``Slice{:tact}`][slice]中不包含[`Int{:tact}`][int]时，尝试加载此类[`Int `][int]会产生异常，[退出码为 8](/book/exit-codes#8)：`Cell overflow`。

尝试加载的数据超过 [`Slice{:tact}`][slice] 所包含的数据时，会出现 [exit code 9](/book/exit-codes#9) 异常：`Cell overflow`。
...
:::note[Useful links:]
  ...
  [特殊的 "coin" 序列化类型](/book/integers#serialization-coins)
...
当[`Address{:tact}`][p][[`Slice{:tact}`][slice]不包含该地址时，尝试加载该[`地址 `][p]会产生异常，[退出码 8](/book/exit-codes#8)：`Cell overflow`。
...
当 [`Cell{:tact}`][cell][slice{:tact}\`][slice]不包含该引用时，尝试加载该引用会产生异常，[退出码 8](/book/exit-codes#8)：`Cell overflow`。
...
试图在[[`Slice{:tact}`][cell] 时预加载此引用 [`slice{:tact}`][slice] 不包含一个异常[退出码 8](/book/exit-codes#8)：`Cell overflow`。

试图预加载更多数据超过 [`Slice{:tact}`][slice] 含有异常[export code 9](/book/exit-codes#9)：`Cell underflow`。
...
检查 [`Slice{:tact}`][slice] 是否为空（即不包含数据位和cell引用）。 如果不是，则抛出异常[退出码 9](/book/exit-codes#9)：`Cell underflow`。
...
试图传递布局与指定 [Struct][struct] 不同的 [`cell{:tact}`][cell]，或加载的数据超过 [`cell{:tact}`][cell] 所包含的数据时，会出现 [exit code 9](/book/exit-codes#9)异常：`Cell underflow`。
...
尝试传递布局与指定 [结构][struct] 不同的 [`Slice{:tact}`][slice]，或加载比 [`Slice{:tact}`][slice] 包含的数据更多的数据时，会出现[退出码 9](/book/exit-codes#9)异常：`Cell underflow`。
...
尝试传递布局与指定 [Message][message] 不同的[`cell{:tact}`][cell]，或加载的数据超过[`cell{:tact}`][cell]所包含的数据时，会出现[退出码 9](/book/exit-codes#9)的异常：`Cell underflow`。
...
试图传递布局不同于指定 [Message][message] 的[`Slice{:tact}`][slice]，或加载的数据多于[`Slice{:tact}`][slice]所包含的数据时，会出现[退出码 9](/book/exit-codes#9)的异常：`Cell underflow`。
