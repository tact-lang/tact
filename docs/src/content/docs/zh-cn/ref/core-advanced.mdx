...
请注意，`cells`和`bits`的数值被用来作为它们最大的数值加上 $1$的模块。 也就是说，如果指定的值高于[账户状态限制（`max_acc_state_cells`和`max_acc_state_bits`）](/book/exit-codes#50)中列出的值，其结果将与指定精确限制的结果相同。 此外，请确保您考虑到[与同一哈希的 cells 分离][deduplication]。

试图指定负数的 `cells`、`bits` 或 `seconds` 会抛出异常[export code 5](/book/exit-codes#5)：\`整数超出预期范围'。
...
试图指定一个 `gassUsed` 的负值导致异常[退出码 5](/book/exit-codes#5)：`Integer out of expected range`。
...
类似于[`getComputeFee(){:tact}`](#getcomputedefee)，但没有[`flat_gas_price`][param-20-21]，如果`gassUsed`小于一个叫做[`flat_gas_limit`][param-20-21] 的阈值，则不支付最低价格。 计算并只返回当前 gas 价格的 `gasUsed` 倍数。
...
[`Context{:tact}`](/ref/core-common#context) 的扩展函数。
...
请注意，`cells` 和 `bits` 的值取其最大值加上 $1$。 也就是说，指定高于[账户状态限制（`max_msg_cells` 和 `max_msg_bits`）](/book/exit-codes#50)中所列值的值，会产生与指定精确限制相同的结果。
...
试图指定负数的 `cells` 或 `bits` 会导致异常[退出码 5](/book/exit-codes#5)：`Integer out of expected range`。
...
试图指定一个 `fwdFee` 的负值导致异常[退出码 5](/book/exit-codes#5)：`Integer out of expected range`。
...
:::note
  ...
  [`getElectorAddress(){:tact}`](/ref/stdlib-config#getconfigaddress)，用于获取选区 [`Address{:tact}`][p]。
...
使用 [`nativeRandomizeLt(){:tact}`](#nativerandomizelt)准备随机数生成器。 由 [`randomInt(){:tact}`](/ref/core-random#randomint) 和 [`random(){:tact}`](/ref/core-random#random) 函数自动调用。
...
[排序消息](/book/send#outbound-message-processing)将通过指定完整的 `cell` 和 [message `mode`](/book/message-mode) 发送。
...
要为 `mode` 参数创建 [`Int{:tact}`][int] 值，只需通过应用 [按位或操作](/book/operators#binary-bitwise-or) 将基本模式与可选标志结合起来：
...
尝试传递具有不同布局的 [`Slice{:tact}`][slice]，或者加载的数据超过给定 [`Slice{:tact}`][slice] 所包含的数据，会抛出异常，带有 [退出码 9](/book/exit-codes#9)：`Cell underflow`。
...
| 字段          | 类型                           | 函数 "raw_reserve "大致等同于创建一个向外发送的消息，将指定的 "金额"[nanoToncoins](/book/integers#nanotoncoin)（或 "b " $-$ "金额"[nanoToncoins](/book/integers#nanotoncoin)，其中 "b "为余额）发送给自己。 这就确保了后续产出行动所花费的资金不会超过剩余资金。 |
| :---------- | :--------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `workchain` | [`Int as int32{:tact}`][int] | 变量长度地址的 Workchain ID                                                                                                                                                                         |
| `address`   | [`Slice{:tact}`][slice]      | 指定的`工作链`中的地址                                                                                                                                                                                 |

尝试传递具有不同布局的 [`Slice{:tact}`][slice]，或者加载的数据超过给定 [`Slice{:tact}`][slice] 所包含的数据，会抛出异常，带有 [退出代码 9](/book/exit-codes#9)：`Cell underflow`。
